...
public class DateUtil extends PropertyEditorSupport {
	...
	public static final SimpleDateFormat time_sdf2 = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
	...
	public static final SimpleDateFormat sdf2 = new SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.US);
	public static final SimpleDateFormat sdgl2 = new SimpleDateFormat("EEE MMM dd yyyy HH:mm:ss z", Locale.ENGLISH);

	public static final String date_sdf_str = "yyyy-MM-dd HH:mm:ss";
	// 以毫秒表示的时间
    ...
    public static Date addDateForWork(Date date, Integer timeLimit) {
        Date calDate = LimitDateCalc.dateCalcWork(date,timeLimit);
        return calDate;
    }
	 ...
	 * @return
	 */
	public static Date getBeginAYear() {
	...
	* @throws
     ...
     *
     */
    public static Date getBenYueFirstDay(Date date) {
        if (date == null) {
            date = new Date();
        }
        date = DateUtil.str2Date(DateUtil.formatDate(date) + " 00:00:00", DateUtil.datetimeFormat);
	 ...
	 *
	 */
	public static Date getBenYueLastDay(Date date) {
		if (date == null) {
			date = new Date();
		}
		date = DateUtil.str2Date(DateUtil.formatDate(date) + " 23:59:59", DateUtil.datetimeFormat);
  ...
  //量day格式一致
    ...
    * @throws
	 ...
	 * @return Date
	 */
	public static Date  getExpireDate(Date startTime, int promiseLimit, List<SysHoliday> holidays) {
		Date flag = startTime;//设置循环开始日期
		Date endTime = DateUtil.addDate(startTime,promiseLimit) ;
		...
		try {
			if(holidays !=null){
				for (SysHoliday holiday: holidays) {
					if("W".equals(holiday.getHolidayType())){
						workdays +=holiday.getHolidayDate()+",";
					}else{
						days +=holiday.getHolidayDate()+",";
					}
				}
			}
		} catch (Exception e) {
		...
		//循环遍历每个日期
		while (!compareDate(flag,endTime)) {
			startCal.setTime(flag);
			//判断是否为周六日
			int week = startCal.get(Calendar.DAY_OF_WEEK);
			if (week == 1 || week == 7) {//1为周日，7为周六
				endTime = DateUtil.addDate(endTime,1);
				//如果该日期是周末且是设定的工作日
				if(workdays.contains(date2Str(flag,date_sdf))){
					endTime = DateUtil.addDate(endTime,-1);
				}
			}else{
				//如果该日期不是周末且是节假日
				if(days.contains(date2Str(flag,date_sdf))){
					//结束日期往后加一天
					endTime = DateUtil.addDate(endTime,1);
				}
			}
			//比较日期往后加一天
			flag =  DateUtil.addDate(flag,1);
		}
		endCal.setTime(endTime);
		//判断结束是否为周六日
		int endweek = endCal.get(Calendar.DAY_OF_WEEK);
		if(!workdays.contains(date2Str(endTime,date_sdf)) || !days.contains(date2Str(endTime,date_sdf))){
			if(endweek == Calendar.SATURDAY) {//周六
				endTime = DateUtil.addDate(endTime,2);
			}
			if(endweek == Calendar.SUNDAY) {//0为周日
				endTime = DateUtil.addDate(endTime,1);
			}
		}
		return endTime;
	...
	public static String startDateFormat(String date) {
		return DateUtil.date2Str(DateUtil.str2Date(date),DateUtil.datetimeFormat);
	}

	public static String endDateFormat(String date) {
		return DateUtil.date2Str(DateUtil.addDate(DateUtil.str2Date(date),1),DateUtil.datetimeFormat);
	}
	 ...
	 * @return
	 */
	public static int calLastedTime(Date startDate,Date nowDate) {
		long nowDatetime = nowDate.getTime();
		long startDateTime = startDate.getTime();
		int diffSeconds = (int) ((nowDatetime - startDateTime) / 1000);
		return diffSeconds;
	}
	 ...
	 *
	 *
	 * @return 当天的17:00:00点时间
	 */
	public static Date getTodaySeventeenTime(Date serviceBeginTime) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(serviceBeginTime);
	 ...
	 * 获取当前时间，前5分钟
	 */
	public static Date beforeFiveMinute(){
		Calendar myTime = Calendar.getInstance();
		myTime.add(Calendar.MINUTE,-5);
		Date beforeM = myTime.getTime();
//		System.out.println("当前时间前5分钟"+beforeM);
		return beforeM;
	}
	/**
	 *
	...
	/**
	 * @return 获取日期的七天前的零点时间
	 */
	public static Date get7DaysStartTime(Date date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
		calendar.set(Calendar.HOUR_OF_DAY, 0);
		calendar.set(Calendar.MINUTE, 0);
	...
	/**
	 * 获取修改后的日期，
	 * @param dayNumb 修改天数，，几天前的零点时间
	 * @return
	 */
	public static Date getTimeByDayNumb(int dayNumb) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(new Date());
		calendar.set(Calendar.HOUR_OF_DAY, 0);
		calendar.set(Calendar.MINUTE, 0);
		calendar.set(Calendar.SECOND, 0);
		calendar.add(Calendar.DATE, dayNumb);
		return calendar.getTime();
	}
	...
	public static Date getNYR() {
		...
		String date = DateUtil.dateformat(dayStr, "yyyy-MM-dd");
		Date workDate = DateUtil.str2Date(date, DateUtil.date_sdf);
		return workDate;
	}
	 ...
	 * @return 获取日期的零点时间
	 */
	public static Date get7DaysEndTime(Date date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
		calendar.set(Calendar.HOUR_OF_DAY, 23);
		calendar.set(Calendar.MINUTE, 59);
	 ...
	 * @return 字符串日期的转标准化
	 */
	public static Date getFormatByStringTime(String dateStr) {
		SimpleDateFormat sdf =   new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss" );
		try {
	...
	public static void main(String[] args) {
		/*int millisDiff = DateUtil.dateDiff('m',DateUtil.getEndTime(),new Date());
		...
		/*String datdString = "Thu Dec 01 2022 09:49:20 GMT 0800 (中国标准时间)";
		try {
			System.out.println(datdString.contains("(中国标准时间)"));
			Date dateTrans = sdgl2.parse(datdString);
			System.out.println(dateTrans);
			String m = "2022-12-28T02:23:01.142Z";
			System.out.println(DateUtil.sdgl2.parse(m));
			Date md = DateUtil.date_sdf.parse(m);
			System.out.println(md);
		} catch (ParseException e) {
			e.printStackTrace();
		}*/
		try {
			String dateString = "2018-02-23 17:30:59";
			Date o= date_sdf.parse(dateString);
			System.out.println(dateString);
			Date p = getTodaySeventeenTime(o);
