...
public class StringUtil {
    ...
    private static Pattern ic18Pattern=Pattern.compile("^[1-9]\\d{5}[1-9]\\d{3}((0\\d)|(1[0-2]))(([0|1|2]\\d)|3[0-1])\\d{4}$");
    private static Pattern urlPattern=Pattern.compile("(https?://(w{3}\\.)?)?\\w+\\.\\w+(\\.[a-zA-Z]+)*(:\\d{1,5})?(/\\w*)*(\\??(.+=.*)?(&.+=.*)?)?");
    ...
    /**
     * 判断是否是URL地址
     * 格式：http://www.zhuofansoft.com:99 或 http://www.zhuofansoft.com
     * @param src
     *            源字符串
     * @return 是否数字的标志
     */
    ...
    private static Pattern transferPattern = Pattern.compile("&#\\d+;");

    public static String transfer(String str) {
        //Pattern p = Pattern.compile("&#\\d+;");
        //Matcher m = p.matcher(str);
        Matcher m = transferPattern.matcher(str);
        while (m.find()) {
            String old = m.group();
    ...
    private static Pattern isDigitPattern = Pattern.compile("[0-9]{1,}");
    // 判断一个字符串是否都为数字
    ...
    private static Pattern getNumbersPattern = Pattern.compile("\\d+");
    // 截取数字
    ...
    private static Pattern splitNotNumberPattern = Pattern.compile("\\D+");
    // 截取非数字
     ...
     * @return
     */
    public static String StringDuplication(String string) {
        //创建一个数组用于存放没有重复的字符串
        List list = new ArrayList();
        for (int i = 0; i < string.length(); i++) {
            //如果list已经有该字符了，则调到下一次循环
     ...
     * @return
     */
    public static String[] ArrayDuplication(String[] string) {
		List list = Arrays.asList(string);
		Set set = new HashSet(list);
		string = (String [])set.toArray(new String[0]);
    	return string;
    }
