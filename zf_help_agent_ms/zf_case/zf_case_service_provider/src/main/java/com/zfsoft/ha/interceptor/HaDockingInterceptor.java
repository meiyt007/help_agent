 ...
 */
@Slf4j
public class HaDockingInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        log.info("执行了第三方对接拦截器的preHandle方法");
        try {
            ApiResultSet<Object> resultSet = new ApiResultSet<>();
            //统一拦截（查询当前redis是否存在user）(这里user会在每次登录成功后，写入redis)
            String token = request.getHeader("token");
            log.info("执行第三方对接拦截器的preHandle方法获取当前用户token信息:{}", token);
            if (token != null && !"".equals(token)) {
                //从redis中取出登录user信息
                String userName = (String) redisTemplate.opsForValue().get(RedisConstants.LOGIN_DOCKING_SESSION_TOKEN+token);
                log.info("执行第三方对接拦截器的preHandle方法获取当前用户信息:{}", userName);
                if (userName != null) {
                    //将登录用户信息存入ThreadLocal
                    HaDockingHolder.addCurrentUser(userName);
//                    redisTemplate.expire(RedisConstants.LOGIN_SESSION_TOKEN+token,RedisConstants.LOGIN_SESSION_TTL, TimeUnit.HOURS);
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
        ...
        //避免内存泄漏
        HaDockingHolder.removeHaThreadLocal();
    }
