...
public class GenDataTreeUtil {
     ...
     * 资源的前缀
     **/
    public final static String TYPE_RESOURCE = "HA-RESOURCE-";
     ...
     * 构建前端所需要下拉树结构
     *
     * @param haUserResources 资源数据
     * @return 下拉树结构列表
     * @author: wuxx
     * @Date: 2020/9/9 15:15
     **/
    public static List<TreeSelect> buildHaUserResourceTreeSelect(List<HaUserResource> haUserResources) {
        List<TreeSelect> treeSelectList = haUserResources.stream().map(haUserResource -> {
            TreeSelect treeSelect = new TreeSelect();
            treeSelect.setId(haUserResource.getId()+"");
            treeSelect.setLabel(haUserResource.getName());
            String parentOid = ObjectUtil.isNotEmpty(haUserResource.getParentId()) ? haUserResource.getParentId().toString() : "";
            treeSelect.setParentId(parentOid);
            treeSelect.setChildren(new ArrayList<>());
            treeSelect.setIdentity("RESOURCE");
