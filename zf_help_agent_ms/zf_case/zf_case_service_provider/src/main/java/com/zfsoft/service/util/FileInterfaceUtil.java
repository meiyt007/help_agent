...
public class FileInterfaceUtil {
    ...
    // 请求地址
    private static String UP_DOWN_URL;

    private static String OTHER_URL;

    // 系统Key
    @Value("${zfsoft.filesystem.apiKey}")
    private String ymlApiKey;
    // 密文
    @Value("${zfsoft.filesystem.publicKey}")
    private  String ymlPUBLIC_KEY;
    // 系统私钥（用于加密和签名）
    @Value("${zfsoft.filesystem.privateKey}")
    private  String ymlPRIVATE_KEY;
    // 请求地址
    @Value("${zfsoft.filesystem.updownURL}")
    private  String ymlUP_DOWN_URL;

    @Value("${zfsoft.filesystem.otherUrl}")
    private  String ymlOTHER_URL;
    ...
    public void getApiKey(){
        ApiKey = this.ymlApiKey;
    }
    //利用@PostConstruct将yml中配置的值赋给本地的变量
    @PostConstruct
    public void getPUBLIC_KEY(){
        PUBLIC_KEY = this.ymlPUBLIC_KEY;
    }
    //利用@PostConstruct将yml中配置的值赋给本地的变量
    @PostConstruct
    public void getPRIVATE_KEY(){
        PRIVATE_KEY = this.ymlPRIVATE_KEY;
    }
    //利用@PostConstruct将yml中配置的值赋给本地的变量
    @PostConstruct
    public void getUP_DOWN_URL(){
        UP_DOWN_URL = this.ymlUP_DOWN_URL+URL_SFX;
    }
    //利用@PostConstruct将yml中配置的值赋给本地的变量
    @PostConstruct
    public void getOTHER_URL(){
        OTHER_URL = this.ymlOTHER_URL+URL_SFX;
    }

    //resource文件夹内读取
    /*static {
        String fileName = "fileSystem.properties";
        Properties props = new Properties();
        try {
            props.load(new InputStreamReader(PropertiesUtil.class.getClassLoader().getResourceAsStream(fileName), "UTF-8"));
            ApiKey = props.getProperty("apiKey");
            PUBLIC_KEY = props.getProperty("publicKey");
            PRIVATE_KEY = props.getProperty("privateKey");
            UP_DOWN_URL = props.getProperty("updownURL");
            OTHER_URL = props.getProperty("otherUrl");
        } catch (Exception e) {
    ...
    public static String upLoadFile(Map<String, String> param, InputStream fis) throws Exception {
        ...
        HttpEntity responseEntity = createFileInterfacePost(UP_DOWN_URL + "/upLoad.rest", requestData, builder.build());
    ...
    public static InputStream downLoadFile(String requestData) throws Exception {
        ...
        HttpEntity responseEntity = createFileInterfacePost(UP_DOWN_URL + "/download.rest", requestData, null);
        if (responseEntity != null) {
    ...
    public static boolean delFile(String requestData) {
        requestData = requestData.replaceAll("//", "/");
        HttpEntity responseEntity = createFileInterfacePost(OTHER_URL + "/delFile.rest", requestData, null);
        if (responseEntity != null) {
    ...
    public static String sendPost(String url, String requestData) throws Exception {
        HttpEntity responseEntity = createFileInterfacePost(OTHER_URL + url, requestData, null);
        if (responseEntity != null) {
            String result = FileInterfaceUtil.responseToString(responseEntity);
            log.info(result);
            return result;
    ...
    public static Map<String, String> genrateParam(String requestData) throws Exception {
        ...
        // 参数传递顺序不可打乱，防止验签错误
        String RequestData = SM2Util.sm2Encrypt(PUBLIC_KEY ,requestData);
        params.put("RequestData", RequestData );
        // 时间戳 , 五分钟过期
        String timeStamp = String.valueOf(System.currentTimeMillis());
        String dataSign = SM2Util.sm2Sign(PRIVATE_KEY , ApiKey , timeStamp );
    ...
    public static Map<String, String> createFileInfoMap(String fileOid, String fileName, String fileOriginalName, String filePath
        ...
        fileParam.put("userType", StrUtil.isNotEmpty(userType) ? userType : "SYSTEM");
