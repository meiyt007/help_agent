...
public class FileUtil {
    ...
    public static int copyFile(File fromFile, File toFile) throws FileNotFoundException, IOException {
        InputStream is = new FileInputStream(fromFile);
        OutputStream os = new FileOutputStream(toFile);
        int ret = copyStream(is, os);
        is.close();
    ...
    //解析文件要素保存内容，获取文件名称
    public static String getFileNameByVal(String val){
        String name = "";
        if(StrUtil.isNotBlank(val)){
            String[] filsStrs = val.split(",");
            for(String filsStr : filsStrs){
                String[] arr = filsStr.split("\\|");//特殊字符使用转义字符转义
                if(arr.length >= 5){
                    if(name != ""){
                        name = name+","+arr[1];
                    }else{
