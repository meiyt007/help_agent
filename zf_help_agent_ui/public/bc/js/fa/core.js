...
function getinit(){
  ...
  var url = VUE_APP_ZC_ROUTE_PATH+"/faModelTemplate/initFaModelTemplate";
  var data = {
  ...
  $.get( url, data, function(data, textStatus, jqXHR){
    ...
    fromJsp.cataOid = data.data.cataOid;
    fromJsp.materialCatalogOid = data.data.cataOid;
...
core.showMetaDataPanel = function(block_id){
	...
	for(var i=0;i<metaDataJsonArray.length;i++){
		...
		var oid = metaData.materialCatalogElementOid;
		var name = metaData.elementName;
		for(var j=0;j<block_array.length;j++){
			var b = block_array[j];
			var meta_data = b.meta_data;
			var meta_data_oid = meta_data ? meta_data.materialCatalogElementOid : null;
...
core.initBlock = function(block_id){
  ...
  /*block_id_1_md*/
	...
	var meta_data = b.meta_data;
	if(meta_data){
		$('#' + block_id + '_md').text(meta_data.elementName);
	}
...
core.loadBlockArray = function(java_block_list_json_str,java_template_json_str){
  ...
  if(java_block_list_json_str != ''&& java_block_list_json_str != 'null'){
    for(var i = 0;i < a.length;i++){
      var java_block = a[i];
      //alert(JSON.stringify(java_block))
      var block = new Block();
...
function saveBlockInfo(block_id){
	...
	var meta_data = get_meta_data(old_block.meta_data.materialCatalogElementOid)
	var b = new Block();
	...
	ztree_helper.updateNode(block_id,meta_data.materialCatalogElementOid);
