...
var Block = function () {
    ...
    this.sort;
    // 打印css样式
    // 'font-family','font-size','font-weight:bold','font-style:italic','text-align','overflow-wrap'
    this.print_css_json_obj = {'font-family': '宋体', 'text-align' : 'left', 'overflow-wrap' : 'break-word'};
    // 临时对象
    this.temp_print_css_json_obj = {};
...
var block_helper = {
    ...
    update_block: function (a, block) {
        for (var i = 0; i < a.length; i++) {
            ...
            if (b.block_html_id == block.block_html_id) {
                ...
                if (block.print_css_json_obj)
                    b.print_css_json_obj = block.print_css_json_obj;
                break;
            }
        }
    },
  ...
  to_java_block : function(block){
    ...
    java_block.marqueeJsonValue = JSON.stringify(block.print_css_json_obj);
    java_block.printCssJsonStr = JSON.stringify(block.print_css_json_obj);
    return java_block;
  },
    to_java_blockold: function (block) {
        var java_block = {};
        java_block.oid = block.block_oid;
        java_block.blockCode = block.block_code;
        java_block.blockName = block.block_name;
        ...
        java_block.sort = block.sort;
        if (block.is_saved && block.meta_data) {
            java_block.metadataOid = block.meta_data.oid;
        }
        java_block.printCssJsonStr = JSON.stringify(block.print_css_json_obj).replace(/"/g, '\'');
        return java_block;
    },
