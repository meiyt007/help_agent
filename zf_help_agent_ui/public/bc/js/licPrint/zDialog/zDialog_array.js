...
Dialog.prototype.create = function () {
	...
	var html = new StringBulider();
	html.append('<table id="_DialogTable_{thisID}" width="')
	.append(this.Width + 26)
	.append('" cellspacing="0" cellpadding="0" border="0" style="font-size:12px; line-height:1.4;border-collapse: collapse;">');

	if(this.borderFlag) {
		html.append('<tr id="_Draghandle_{thisID}" ondblclick="Dialog.changeWinSizeFun()" onselectstart="return false;"  style="-moz-user-select: -moz-none; ')
			.append(this.Drag ? "cursor: move;" : "")
			.append('">')
			.append('<td width="13" height="33" style="background-image: url({IMAGESPATH}dialog_lt.png) !important;background: url({IMAGESPATH}dialog_lt.gif) no-repeat 0 0;"><div style="width: 13px;"></div></td>')
			.append('<td height="33" style="background-image:url({IMAGESPATH}dialog_ct.png) !important;background: url({IMAGESPATH}dialog_ct.gif) repeat-x top;"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;"><span id="_Title_{thisID}">')
			.append(this.Title)
			.append('</span></div>')
			.append('<div id="_ButtonClose_{thisID}" onclick="Dialog.getInstance(\'{thisID}\').cancelButton.onclick.apply(Dialog.getInstance(\'{thisID}\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url({IMAGESPATH}dialog_closebtn.gif)\'" onmouseover="this.style.backgroundImage=\'url({IMAGESPATH}dialog_closebtn_over.gif)\'" style="margin: 4px 0 0;*margin-top: 5px; position: relative;top:auto; cursor: pointer; float: right; height: 17px; width: 28px; background: url({IMAGESPATH}dialog_closebtn.gif) 0 0;')
			.append(ielt7 ? "margin-top: 3px;" : "")
			.append(this.ShowCloseButton ? "" : "display:none;")
			.append('"></div>')
			.append('<div id="_ButtonWinSize_{thisID}" onclick="Dialog.changeWinSizeFun()" style="margin: 4px 0 0;*margin-top: 5px; position: relative;top:auto; cursor: pointer; float: right; height: 17px; width: 28px; background: url({IMAGESPATH}icon_max.gif) 0 0;')
			.append(ielt7 ? "margin-top: 3px;" : "")
			.append(this.ChangeWinSize ? "" : "display:none;")
			.append('"></div></td>')
			.append('<td width="13" height="33" style="background-image: url({IMAGESPATH}dialog_rt.png) !important;background: url({IMAGESPATH}dialog_rt.gif) no-repeat right 0;"><div style="width: 13px;"><a id="_forTab_{thisID}" href="#;"></a></div></td>')
			.append('</tr>')
			.append('<tr valign="top">');
  	} else {
  		html.append('<tr id="_Draghandle_{thisID}">')
			.append('<td width="13" height="33"><div style="width: 13px;"></div></td>')
			.append('<td height="33" style="border-bottom: 1px solid #6cabe2;"></td>')
			.append('<td width="13" height="33"><div style="width: 13px;"><a id="_forTab_{thisID}" href="#;"></a></div></td>')
			.append('</tr><tr valign="top">');
  	}
    if(this.borderFlag) {
    	html.append('<td width="13" style="background-image: url({IMAGESPATH}dialog_mlm.png) !important;background: url({IMAGESPATH}dialog_mlm.gif) repeat-y left;"></td>');
    } else {
    	html.append('<td width="13" style="border-right: 1px solid #6cabe2;"></td>');
    }
	html.append('<td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">')
		.append('<tr id="_MessageRow_{thisID}" style="')
		.append(this.ShowMessageRow ? "" : "display:none")
		.append('"><td valign="top" height="50"><table width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#eaece9 url({IMAGESPATH}dialog_bg.jpg) no-repeat scroll right top;" id="_MessageTable_{thisID}">')
		.append('<tr>')
		.append('<td width="50" height="50" align="center"><img width="32" height="32" src="{IMAGESPATH}' + this.MessageIcon + '" id="_MessageIcon_{thisID}"/></td>')
		.append('<td align="left" style="line-height: 16px;"><div id="_MessageTitle_{thisID}" style="font-weight:bold">' + this.MessageTitle + '</div>')
		.append('<div id="_Message_{thisID}">' + this.Message + '</div></td>')
		.append('</tr></table></td></tr><tr>')
		.append('<td valign="top" align="center"><div id="_Container_{thisID}" style="position: relative; width: ' + this.Width + 'px; min-height: ' + this.Height + 'px; height: ' + this.Height + 'px;">')
		.append('<div style="position: absolute; height: 100%; width: 100%; display: none; background-color:#fff; opacity: 0.5;" id="_Covering_{thisID}">&nbsp;</div>')
		.append((function (obj) {
			if (obj.InnerHtml) return obj.InnerHtml;
			if (obj.URL) return '<iframe width="100%" height="100%" frameborder="0" style="border:none 0;" scrolling="no" id="_DialogFrame_' + obj.ID + '" ' + (obj.InnerFrameName?'name="'+obj.InnerFrameName+'"':'')+' src="' + obj.displacePath() + '"></iframe>';
			return "";
		})(this))
		.append('</div></td></tr>')
		.append('<tr id="_ButtonRow_{thisID}" style="' + (this.ShowButtonRow ? "" : "display:none") + '">')
		.append('<td height="36"><div id="_DialogButtons_{thisID}" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">')
		.append('<input type="button" class="buttonStyle" value="' + this.OkButtonText + '" id="_ButtonOK_{thisID}"/>')
		.append('<input type="button" class="buttonStyle" value="' + this.CancelButtonText + '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>')
		.append('</div></td></tr></table></td>');
	if(this.borderFlag) {
		html.append('<td width="13" style="background-image: url({IMAGESPATH}dialog_mrm.png) !important;background: url({IMAGESPATH}dialog_mrm.gif) repeat-y right;"></td>');
	} else {
		html.append('<td width="13" style="border-left: 1px solid #6cabe2"></td>');
	}
	html.append('</tr><tr>');
	if(this.borderFlag) {
		html.append('<td width="13" height="13" style="background-image: url({IMAGESPATH}dialog_lb.png) !important;background: url({IMAGESPATH}dialog_lb.gif) no-repeat 0 bottom;"></td>')
			.append('<td style="background-image: url({IMAGESPATH}dialog_cb.png) !important;background: url({IMAGESPATH}dialog_cb.gif) repeat-x bottom;"></td>')
			.append('<td width="13" height="13" style="background-image: url({IMAGESPATH}dialog_rb.png) !important;background: url({IMAGESPATH}dialog_rb.gif) no-repeat right bottom;"><a onfocus=\'$id("_forTab_{thisID}").focus();\' href="#;"></a></td>');
	} else {
		html.append('<td width="13" height="13"></td>')
			.append('<td style="border-top: 1px solid #6cabe2"></td>')
			.append('<td width="13" height="13"><a onfocus=\'$id("_forTab_{thisID}").focus();\' href="#;"></a></td>');
	}
	html.append('</tr></table></div>');
	var xsHtml=html.toString("").replace(/\{IMAGESPATH\}/gm,IMAGESPATH).replace(/\{thisID\}/gm,this.ID)
	...
	//alert("create:" + (endDate - beginDate));
};
...
Dialog.alert = function (msg, func, w, h, drag) {
    ...
    diag.InnerHtml = '<table height="100%" width="90%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" style="padding-left:20px;padding-top:10px;padding-bottom:10px;font-size:16px; font-family: 微软雅黑" id="Message_' + this.ID + '">' + msg + '</td></tr>\
	</table>';
    diag.show();
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h, type) { // type 1 提示 2 警告
    ...
    if(type != null && type=='1') tsimg = 'icon_alert.gif';
    ...
    diag.InnerHtml = '<table height="100%" width="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right" style="width:15%"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + tsimg+'" width="34" height="34" align="absmiddle"></td>\
			<td align="left" style="padding-left:20px; padding-right:5px;font-size:16px; font-family: 微软雅黑;" id="Message_' + this.ID + '">' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.focus();
