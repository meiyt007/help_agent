...
var PopupLayer = new Class({
	...
	_construct:function(){                  //构造弹出层
		...
		this.popupLayer.append(this.popupBlk.css({opacity:1})).appendTo($(document.body)).css({position:"absolute",'z-index':1000,height:this.popupBlk.get(0).offsetHeight});
	...
	_loadOverlay:function(){                //加载遮罩
		...
		this.overlay.css({position:"absolute","z-index":1,left:0,top:0,zoom:1,display:"none",height:$(document).height()}).appendTo($(document.body)).append("<div style='position:absolute;z-index:1000;width:100%;height:100%;left:0;top:0;opacity:0.3;filter:Alpha(opacity=30);background:#000'></div><iframe frameborder='0' border='0' style='width:100%;height:100%;position:absolute;z-index:1;left:0;top:0;filter:Alpha(opacity=0);'></iframe>")
	},
	doresize:function(){
		...
		this.overlay?this.overlay.css({height:($.browser.version=="6.0")?$(document).height()-4:$(document).height()}):null;
		if(this.isSetPosition){
			this.setPosition(this.trigger.offset().left + this.options.offsets.x, this.trigger.offset().top + this.trigger.get(0).offsetHeight + this.options.offsets.y);
		}
	},
	...
	recalculatePopupIframe:function(){     //重绘popupIframe,这个不知怎么改进，只好先用这个笨办法。
		...
		this.popupIframe.css({position:"absolute",'z-index':-1,left:0,top:0,opacity:0,height:this.popupBlk.get(0).offsetHeight});
	},
	close:function(){                      //关闭方法
