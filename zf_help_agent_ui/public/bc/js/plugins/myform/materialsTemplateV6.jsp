...
<link rel="stylesheet" href="${ctxPath }css/media.css">
<script type="text/javascript" src="${ctxPath }js/plugins/ztree/jquery.ztree.all.min.js"></script>
<script type="text/javascript" src="${ctxPath }js/plugins/signature/jSignature.min.js"></script>
...
<script type="text/javascript">
	...
	function deleteAtta(obj, attaOid) {
		...
		common.ajaxPost(deleteRequestUrl, {
			attaOid : attaOid
		}, function(data) {
			var mes = eval(data);
			if (mes.success) {
				Dialog.alert(mes.data, function() {
	...
	function outputData(divId){
		var signatureDivId="#"+divId;
		var dataId="#data_"+divId;
		var datapair = $(signatureDivId).jSignature("getData", "base30"); 
		$(dataId).val(datapair);
	}
	function resetSelf(divId){
		var signatureDivId="#"+divId;
		$(signatureDivId).jSignature("reset");
	}
	function replay(divId){
		var signatureDivId="#"+divId;
		var dataId="#data_"+divId;
		var data=$(dataId).val();    	
		var dataurl='data:'+ data;
		$(signatureDivId).jSignature("importData", dataurl);
	}
	end85889093
</script>
<script type="text/javascript">
