...
define('plupload/FileUploader', [
	...
	function FileUploader(fileRef, queue) {
		...
		plupload.extend(this, {
            ...
            */
			...
			start: function(options) {
				...
				if (self._options.chunk_size) {//需要分块时执行
					...
					if( _totalChunks > 1 ) {
						$.ajax({
							type : self._options.http_method,
							async : false,
							...
							success : function(data) {
								_beginChunk = Number(data);
							}
						});  
					}
					
					/** 断点续传end */
					if(isNaN(_beginChunk)) {
						_beginChunk = 0;
					}
					self.uploadChunk(_beginChunk, false, true);
				} else {// 不分块直接上传
			...
			uploadChunk: function(seq, options, dontStop) {
				...
				chunk.seq = parseInt(seq, 10) || getNextChunk(seq);
				...
				_options = plupload.extend({}, this.getOptions(), {
					params: {
						...
						lastModifiedDate: _file.lastModifiedDate.getTime()
					}
				});
				...
				up.bind('progress', function(e) {
					self.progress(calcProcessed() + e.loaded + _beginChunk * chunkSize, _file.size);
				});
				...
				up.bind('done', function(e, result) {
					...
					if (calcProcessed() + _beginChunk * chunkSize >= _file.size) {
						self.done(result); // obviously we are done
					} else if (dontStop) {
						plupload.delay(function() {
							self.uploadChunk(getNextChunk(seq), false, dontStop);
						});
					}
				});
		...
		function getNextChunk(seq) {
			var i = 0;
			if(seq) {
				//while (i < _totalChunks && i <= seq && _chunks.hasKey(i)) {
				while (i < _totalChunks && i <= seq) {
					i++;
				}
			} else {
		...
		switch (option) {
			...
			case 'filters':
				...
				if (value.mime_types) {
					...
					//文件类型会覆盖文件大小的限制，重新赋值
					value.max_file_size = options.max_file_size;
	...
	addFileFilter('mime_types', function(filters, file, cb) {
		if (filters.length && !filters.regexp.test(file.name)) {
			this.trigger('Error', {
				...
				message: plupload.translate('文件类型错误.'),
				file: file
			});
			cb(false);
		} else {
	...
	addFileFilter('max_file_size', function(maxSize, file, cb) {
		...
		if (file.size !== undef && maxSize && file.size > maxSize) {
			this.trigger('Error', {
				...
				message: plupload.translate('文件大小超过限制，最大为' + maxFileSize + '.'),
				file: file
			});
			cb(false);
		} else {
			...
			while (ii--) {
				...
				if (file.name === this.files[ii].name && file.size === this.files[ii].size) {
					this.trigger('Error', {
						...
						message: plupload.translate('文件重复.'),
						file: file
					});
					cb(false);
					return;
