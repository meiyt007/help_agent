...
(function (factory) {
    ...
    function adjustDropMenuPos($input, $dropdownMenu, options) {
        ...
        //列表对齐方式
        var dmcss;
        var left = $input.siblings('div').width() - $input.parent().width();
    ...
    function refreshDropMenu($input, data, options) {
        var $dropdownMenu = $input.parents('.input-group:eq(0)').find('ul.dropdown-menu'),
            len, i, j, index = 0,
            tds,
            html = ['<table class="table table-condensed table-sm">'],
            idValue, keyValue; //作为输入框 data-id 和内容的字段值
        
        if(options.displayType == 'tree') {
            $("#" + options.treeId).css("display", "block");
            $dropdownMenu.show();
        } else {
    ...
    function getIClear($input, options) {
        ...
        if (options.clearable && !$iClear.length) {
                $iClear = $('<i class="clearable fa fa-remove"></i>')
...
//                $input.removeClass('disabled').attr('disabled', false).attr('autocomplete', 'off');
                ...
                $dropdownMenu.on('mouseover', function() {
                        ...
                        //console.log('$dropdownMenu-mouseout')
                        mouseenterDropdownMenu = 0;
                        //$input.focus();
                ...
                if ($iClear.length) {
                    $iClear.click(function () {
                        ...
                        if(options.clearHiddenId) {
                			$("#" + options.clearHiddenId).val('');
                        }
                        if(options.clearTreeId){
                        	$("#" + options.clearTreeId).click();
                        }
                    });
    ...
    $.fn.bsSuggest = function(options) {
    	//验证控件是否已经初始化suggest，如果已经初始化，则不在添加suggest
    	for(var i = 0; i < suggestIds.length; i++) {
    		var sid = suggestIds[i];
    		if(sid == $(this).attr("id")) 
    			return;
    	}
    	
    	suggestIds.push($(this).attr("id"));
 ...
 * 文本框绑定选择树
 * @param treeId 绑定树的ID
 * @param treeData 绑定树的数据
 * @param txtId 触发文本框ID
 * @param txtHiddenId 选择后保存树的节点值
 * @param clearTreeId 清理已选择的节点
 * @param customTreeClick 自定义树的节点点击事件
 */
function txtBindTree(treeId, treeData, txtId, txtHiddenId, clearTreeId, customTreeClick, check, asyncParam, beforeCheck) {
	if(!check) {
		check = false;
	}
	$("#" + txtId).bsSuggest({
		clearable: true,
		clearHiddenId: txtHiddenId,
		clearTreeId: clearTreeId,
		displayType : 'tree'
	});
	if(!asyncParam) {
		asyncParam = {enable: false};
	}
	$.fn.zTree.init($("#" + treeId), {
		...
		data : {
			...
			onClick: function onClick(event, treeId, treeNode){
				if(!check) {	//当为选择框时，不定义节点的事件
					if(customTreeClick) {
						customTreeClick(treeNode);
					}
					
					treeClick(treeNode, txtId, txtHiddenId);
				}
			},
			onCheck: beforeCheck
		}
...
function treeClick(treeNode, txtId, txtHiddenId) {
	$("#" + txtId).val(treeNode.name);
	$("#" + txtHiddenId).val(treeNode.id);
	$("#" + txtId).parent('.input-group').find('ul.dropdown-menu').hide();
}