    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                ...
                editor:this,
                title: '单选框组',
                cssRules:"width:750px;height:370px;",
                buttons:[
                {
                    className:'edui-okbutton',
                    label:'确定',
...
/***文本框按钮注册**/
UE.registerUI('button_input',function(editor,uiName){
    if(!this.options.toolleipi)
    {
        return false;
    }
...
UE.registerUI('button_textarea',function(editor,uiName){
    ...
    var btn = new UE.ui.Button({
        ...
        title:"文本域",
        //需要添加的额外样式，指定icon图标，这里默认使用一个重复的icon
        cssRules :'background-position: -778px -77px;',
        //点击时执行的命令
        onclick:function () {
            //这里可以不用执行命令,做你自己的操作也可
           editor.execCommand(uiName);
...
UE.registerUI('button_select',function(editor,uiName){
    ...
    var btn = new UE.ui.Button({
        ...
        title:"下拉菜单",
        //需要添加的额外样式，指定icon图标，这里默认使用一个重复的icon
        cssRules :'background-position: -800px -77px;',
        //点击时执行的命令
        onclick:function () {
            //这里可以不用执行命令,做你自己的操作也可
           editor.execCommand(uiName);
...
UE.registerUI('button_radios',function(editor,uiName){
    ...
    var btn = new UE.ui.Button({
        ...
        title:"单选框",
        //需要添加的额外样式，指定icon图标，这里默认使用一个重复的icon
        cssRules :'background-position: -825px -77px;',
        //点击时执行的命令
        onclick:function () {
            //这里可以不用执行命令,做你自己的操作也可
           editor.execCommand(uiName);
...
UE.registerUI('button_checkboxs',function(editor,uiName){
            ...
            var btn = new UE.ui.Button({
                ...
                title:"复选框",
                //需要添加的额外样式，指定icon图标，这里默认使用一个重复的icon
                cssRules :'background-position: -849px -77px;',
                //点击时执行的命令
                onclick:function () {
                    //这里可以不用执行命令,做你自己的操作也可
                   editor.execCommand(uiName);
...
/**保存并生成版本号**/
UE.registerUI('button_version',function(editor,uiName){
    if(!this.options.toolleipi)
    {
        return false;
    }
    ...
    editor.registerCommand(uiName,{
        execCommand:function(){
            try {
                formDesign.fnCheckForm('version');
            } catch ( e ) {
                //alert('formDesign.fnCheckForm("version") 保存异常');
            }
    ...
    var btn = new UE.ui.Button({
        ...
        title:"保存并生成表单",
        //需要添加的额外样式，指定icon图标，这里默认使用一个重复的icon
        cssRules :'background-position: -871px -77px;',
        //点击时执行的命令
        onclick:function () {
            //这里可以不用执行命令,做你自己的操作也可
           editor.execCommand(uiName);
