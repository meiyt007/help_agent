function imageLabel(c) {
    ...
    var butHtml = '';
    if(config.showBut){
		butHtml = `<div class="imageLabel-buttons"><div class="imageLabel-btn imageLabel-closes">关闭</div><div class="imageLabel-btn">确定</div></div>`;
	}
    ...
    function next() {
        ...
        $.each(config.data,function(i,o){//渲染默认数据
            $d = $(
                `<div class="imageLabel-imgdrop ${o.name?'imageLabel-drop-has':''}" title="${o.name||''}"><span class="imageLabel-imgdrop-font">${o.name||''}</span><div class="imageLable-i-s"></div></div>`
            );
        ...
        $('.imageLabel-img-boxs').mousemove(function (e) {
            ...
            if (flag) {
                ...
                if (Math.abs(e.clientX - obj.cx) > 10 &&
                	...
                	//获取一共有多少个审查要点
                	var idx = $('.imageLabel-imgdrop').length||0;

                	$d.find('.imageLabel-imgdrop-font').text(idx);

                	var jsonObj = JSON.parse($d.attr('data-json'));
                	jsonObj.name = idx;
                	$d.attr('data-json',JSON.stringify(jsonObj)).attr('title',idx);

                	config.addLable($d,idx);
        ...
        $input.find('.imageLabel-input-ok').click(function () {
            ...
            j.name = $i.val();
            $d.attr('data-json',JSON.stringify(j)).attr('title',j.name);
            $input.removeClass('imageLabel-active');
            if($i.val()){
        ...
        $('.imageLabel-delete').click(function () { //删除操作
        	...
        	//删除的时候 需要把 原来的从新排序 ，比如删除的是2，那么就必须把从3 开始一直到最后的 N 都减去1
        	var delName = parseInt(JSON.parse($d.attr('data-json')).name);
            $d.remove();
            $('.imageLabel-imgdrop').each(function(){
