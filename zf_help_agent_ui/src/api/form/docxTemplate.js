...
var applicationName =
  process.env.VUE_APP_FORM_API_ROUTE_PATH + "/manager/security/docx/template";
...
export function exportFormDataToDocx(query) {
  window.location.href =
    ...
    "/form/manager/exportFormDataToDocx?designAndReportOid=" +
    query.designAndReportOid +
    "&docxTemplateOid=" +
    query.docxTemplateOid +
    "&linkDesignAndReportOids=" +
    query.linkDesignAndReportOids +
    "&isPdf=" +
    query.isPdf;
}
...
export function exportFormDataToDocxPost(query) {
  ...
  let url =
    ...
    "/manager/exportFormDataToDocxPost";
...
function downLoadFile(url, params, fileName) {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4 && xhr.status === 200) {
      try {
        if (!fileName) {
          let decodeFileName = decodeURIComponent(
...
export function getAppendObjects(authorizeKey, moduleOid) {
  ...
  return request({
    url:
      process.env.VUE_APP_FORM_API_ROUTE_PATH +
      "/manager/security/object/queryDataSourceFormObjectList",
    method: "get",
    params: query
  });
}
...
export function getObjectColumns(objectOid) {
  return request({
    url:
      process.env.VUE_APP_FORM_API_ROUTE_PATH +
      "/manager/security/column/queryFormColumnStrByObjectOid?objectOid=" +
      objectOid,
    method: "get"
  });
}
