...
export function pageServiceOidList(comboDirectoryOid) {
  return request({
    url: process.env.VUE_APP_ZC_ONETHING_ROUTE_PATH + '/comboMealCatalogRelated/queryListByComboDirectoryOid?comboDirectoryOid='+comboDirectoryOid,
    method: 'post'
  })
}
...
export function saveOrUpdateComboDirectoryMaterial(data) {
  return request({
    url: process.env.VUE_APP_ZC_ONETHING_ROUTE_PATH + '/comboMealCatalogRelated/saveOrUpdateComboDirectoryMaterial',
    method: 'post',
    headers:{
      "Content-Type":"application/json;charset=UTF-8",
    },
...
// 查询一件事目录公共材料和细化材料列表
export function queryComboRefinedMaterialByComboDireOid(comboDireOid) {
  return request({
    url: process.env.VUE_APP_THING_ROUTE_PATH +'/combo/directoryMaterial/queryComboRefinedMaterialByComboDireOid/'+comboDireOid,
    method: 'get'
  })
}
