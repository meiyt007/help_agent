...
// 获取已选和有权限的区划树信息--districtOidsForParam 为已选择的区划
export function queryDistrictSimpleTreeForSelected(districtOidsForParam,districtOid) {
  if(districtOid == undefined){
    districtOid = '';
  }
  if(districtOidsForParam == undefined){
    districtOidsForParam = '';
  }
  return request({
    url: '/platform/security/common/queryDistrictSimpleTree?districtOid='+districtOid+'&districtOids='+districtOidsForParam,
    method: 'get'
  })
}
...
// 获取用户管理区划树信息
export function queryDistrictTree(districtOid) {
  if(districtOid == undefined){
    districtOid = '';
  }
  return request({
    url: '/platform/security/common/queryDistrictTree?districtOid='+districtOid,
    method: 'get'
  })
}
