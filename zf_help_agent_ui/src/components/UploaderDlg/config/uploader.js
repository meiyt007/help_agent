import config from '@/components/UploaderDlg/config/config';
...
var createPluploadUploader = function (param) {
    let UploadParams = {
        ...
        // 上传文件路径
        url : process.env.VUE_APP_BASE_API + process.env.VUE_APP_SX_ROUTE_PATH + param.ctxPath,
        extensions: "",
        // filters : [{
        //     title : 'files',
        ...
        init : {
            ...
            Error : function(uploader, errObject) {
                if(errObject.code==-601 ){
                  param.error_fun('不允许上传非法后缀名文件！');
                }else {
                  param.error_fun('上传文件不能为空');
                }
            }
        }
    }
