...
export default {
  ...
  components: { Treeselect, applyUserList },
  ...
  data() {
    return {
      ...
      formFieldResult: {},
      enterNum: 0, // 进入的次数
      onlyMountedLater: false,
      socket: null, //高拍仪socket
      dialog: "", //高拍仪身份证
  ...
  watch: {
    ruleFormForWatch: {
      ...
      handler: debounce(function(value, oldValue) {
        // 暂存受理初次进入
        if (this.onlyMounted && !this.onlyMountedLater) {
          this.onlyMountedLater = true;
          return;
        }
        const form = {};
        this.ruleFormMap.forEach(item => {
  ...
  async activated() {
    if (this.rqbzDueDate) {
      this.ruleForm.rqbzDueDate = this.rqbzDueDate;
    }

    // 暂存受理首次进入不请求
    if (!this.onlyMounted || this.enterNum > 0) {
      // 如果是从智审下来的不做请求
      if (
        (this.lastComponentId !== null && this.lastComponentId > 4) ||
        !this.caseOid
      )
        return;
      // 获取电子表单字段
      ...
      if (idCardInfo) {
        ...
        if (this.cegisterType === "0") {
          this.ruleForm.applyUserName =
            idCardInfo.CNName || this.ruleForm.applyUserName;
          this.ruleForm.credentialNumber =
            idCardInfo.carID || this.ruleForm.credentialNumber;
        }

        if (this.cegisterType == "1") {
          this.ruleForm.applyUserName =
            idCardInfo.legalUserName || this.ruleForm.applyUserName;
          this.ruleForm.credentialNumber =
            idCardInfo.legalIdCard || this.ruleForm.credentialNumber;
        }
  ...
  methods: {
    openSocket() {
      ...
      this.socket.onopen = function() {
        ...
        new QWebChannel(that.socket, function(channel) {
          ...
          //发送信号
          // console.log('1111',that.dialog)
          that.dialog.devChanged("primaryDev_:WW690R");
          that.dialog.html_loaded("two");
          //阅读身份证
          // that.dialog.get_actionType("startIDCard")
        });
      };
    },
    ...
    handleUserInfo() {
      if (!this.isShowTipsOfUserInfo || this.onlyMounted) return;
      this.$refs.ruleForm.validateField("credentialNumber", validMessage => {
        if (!validMessage) {
          this.$nextTick(() => {
            this.getUserInfoList().then(({ code, data }) => {
    ...
    handleScanCard(scanType) {
      ...
      if (DEVEICETYPE.IDCARD_CONFIG === ID_CARD_V4) {
        this.getIdcardDatav4(scanType);
        // alert('身份证信息')
      }
    },
    async getIdcardDatav4(scanType) {
      // alert('身份证信息')
      // this.openSocket()
      console.log("shengfenz", this.dialog);
      const that = this;
      //阅读身份证
      console.log("this.dialog====", this.dialog);
      this.dialog.get_actionType("startIDCard");
      this.dialog.get_functionTypes("getIdcardImage", "5", "", "");
      new Promise((resolve, reject) => {
        this.dialog.sendPrintInfo.connect(function(message) {
          ...
          if (message.indexOf("IDcardInfo:") >= 0) {
            ...
            // resolve(that.sfzmessage)
            that.sfzmessage = message;
            resolve(that.sfzmessage);
        ...
        //获取身份证姓名身份证号并读取
        () => {
          // console.log('message', that.sfzmessage)
          that.sfzarry = that.sfzmessage.split(" ");
          that.ruleForm.credentialNumber = that.sfzarry[7];
          // console.log(that.sfzarry[0])
          const namestr = that.sfzarry[0];
          const namearry = namestr.split(":");
          that.ruleForm.applyUserName = namearry[1];
        }
      );
    },
    ...
    getBasicAndFormFieldRel() {
      return getBasicAndFormFieldRel(this.serviceOid).then(({ code, data }) => {
        if (code !== 200) return;
