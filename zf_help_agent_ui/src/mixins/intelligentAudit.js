...
import {
  getQlCaseMaterialListAndAuditResult,
  viewDetailResult,
...
export default {
  ...
  inheritAttrs: false,
  props: ["caseOid", "serviceOid", "ruleForm", "serviceName"],
  ...
  mounted () {
    !this.isKeepAlive && this.getMaterialPrePrialNew();
  },
  methods: {
    ...
    getMaterialPrePrialNew () {
      ...
      getQlCaseMaterialListAndAuditResult({ caseOid: this.caseOid, serviceOid: this.serviceOid })
        // getQlCaseMaterialListAndAuditResult({ caseOid: 'ed18bc82fb8b4f66b20061f75a106663', serviceOid: '2c287b8b79f39489017a190d7f8c13ff' })
        .then(({ code, data, message }) => {
          ...
          this.intelligentList = data.qlCaseMaterials.map((item) => {
            return { ...item };
          });
        })
        .catch(() => this.loading.close());
