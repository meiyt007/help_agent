...
//初始化websocket
function initWebSocket(error) {
  if ("WebSocket" in window) {
    webSocket = new WebSocket('ws://localhost:8181');//创建socket对象
...
function webSocketOnMessage(e) {
  ...
  if (materialIndex != null) {
    globalCallback(data, materialIndex);
  } else {
    globalCallback(data);
  }
...
function webSocketClose() {
  if (webSocket){
    //这句话是关键，之前我忘了写，一直没有真正的关闭socket
    webSocket.close();
    console.log("WebSocket对话连接已关闭");
  }
}


//在其他需要socket地方调用的函数，用来发送数据及接受数据
function sendSock(agentData, callback, index) {
