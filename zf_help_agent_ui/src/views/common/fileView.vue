<template>
  <div>
    <div v-if="showImage == true" class="dialog-body-content-base-style">
      <img width="300px" height="550px"  style="width: 100%;object-fit: contain;" :src="fileUrl">
    </div>
...
<script>
  import { getSysAttaByAttaOid,getPdf } from "@/api/sys/atta";
    ...
    methods: {
      ...
      getAttaNginxPath(){
        if(!this.attaNginxPath){
          if(this.attaOid){
            getSysAttaByAttaOid(this.attaOid).then(response => {
      ...
      preview(ty,name,fileUrl) {
        ...
        if(1 === ty){
          ...
          this.fastFlag ='0';
          this.fileUrl = process.env.VUE_APP_BASE_API + "/platform/security/atta/downloadFile/"+ fileUrl + '?access_token=' +  getToken();
        }
