  ...
  <div class="app-container typeApp">
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['im:devApplyServer:view']"
          >查看</el-button
          >
          <el-button
            size="mini"
            ...
            v-hasPermi="['im:devApplyServer:audit']"
          >审核</el-button
          >
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <div class="zf-zc-table--title">服务申请基本信息</div>
        <table cellspacing="0" cellpadding="0" border="0" class="zf-zc-table">
          ...
          <tr>
            <td><b>应用名称：</b></td>
            <td>
              {{ devApplyServer.appliName }}
            </td>
            <td><b>服务名称：</b></td>
            <td>
              {{ devApplyServer.serverName }}
            </td>
          </tr>
          <tr>
            <td><b>状态：</b></td>
            <td colspan="3">
              {{ devApplyServer.status | statusHandle }}
            </td>
          </tr>
          <tr>
            <td><b>申请理由：</b></td>
        ...
        <div class="zf-zc-table--title">开发商申请服务审核记录列表</div>
        <el-table v-loading="loading" :data="devApplyAuthRecList" border>
    ...
    >
      ...
      <div class="zf-zc-table--title"  >开发商申请服务审核记录列表</div>
      <el-table width="100%" v-loading="loading" :data="devApplyAuthRecList" border>
        <el-table-column label="序号" width="50" type="index" align="center">
          <template slot-scope="scope">
            <span>{{ scope.$index + 1 }}</span>
          </template>
...
import {
  ...
  getOne,
  save,
} from "@/api/middle/devApplyServer";
export default {
  components: {},
  name: "DevApplyServer",
  data () {
    return {
      // 遮罩层
      loading: true,
      statusMap: { "0": "未提交", "1": "待审核", "2": "审核通过", "3": "审核不通过" },
      // 总条数
      total: 0,
      title: "",
      // 数据表格
  ...
  methods: {
    ...
    getList () {
      ...
      page(this.queryParams).then(response => {
        this.devApplyServerList = response.data.datas;
    ...
    handleView (row) {
      ...
      getOne(oid).then(response => {
        this.devApplyServer = response.data.devApplyServer;
        this.middleUser = response.data.devApplyServer.middleUser;
        this.devApplyAuthRecList = response.data.devApplyAuthRecList;
        this.openView = true;
        this.title = "查看应用信息";
      });
    },
    ...
    handleInit (row) {
      ...
      init(oid).then(response => {
        ...
        this.middleUser = response.data.devApplyServer.middleUser;
        this.devApplyAuthRecList = response.data.devApplyAuthRecList;
        this.form.applyOid = response.data.devApplyServer.oid;
        this.openInit = true;
        this.title = "审核";
      });
    },
