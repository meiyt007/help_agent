/** * 材料审查要点配置管理 * @author: chenjm * @date: 2020-11-10 */
<template>
  <div class="app-container">
        ...
        <el-table v-loading="loading" :data="attaList" border>
          ...
          >
            <template slot-scope="scope">
              ...
              <el-link
                type="primary"
                @click="openExaminePointCardingView(scope.row)"
                >预览</el-link
              >
              |
              <el-link
                type="primary"
                @click="openExaminePointCardingManage(scope.row)"
                >编辑</el-link
              >|
              <el-link type="primary" @click="deleteFile(scope.row)"
                >删除</el-link
              >
            </template>
          </el-table-column>
    ...
    <el-dialog
      v-dialog-drag
      title="预览审查要点配置"
      :visible.sync="initPointOptionsView"
      @close="closeFileView"
      width="60%"
      append-to-body
    >
...
<script>
import {
  queryComboDirectoryMaterialList,
  page,
  getSxServiceOne,
  saveOrUpdate,
  getTaocanList,
  saveOrUpdateComboAhsSamplePicInfo,
  getComboAhsSamplePicInfoList,
  deleteComboAhsSamplePicInfo
} from "@/api/onething/clzs/materialCheckKeyManage/comboMaterialCheckKeyManage.js";
...
export default {
  ...
  created() {
    // this.getList();
    this.getServiceTypt();
    this.getTaocanList(); //获取一件事目录列表
    this.queryParams.sShow = false;
    this.queryParams.cShow = true;
    this.queryParams.serviceTypeOidDict = "tcsx";
  },
  methods: {
    getListAll() {
    ...
    peizhi(row, sxOrTc) {
      ...
      this.ahsSamplePicInfo.materiaOid = row.materialOid;
      aspi.materiaOid = row.materialOid;
      this.getaspiList(this.ahsSamplePicInfo);
    },
    //获取样表列表
    getaspiList(aspi) {
      getComboAhsSamplePicInfoList(aspi).then(response => {
    ...
    handleInitTc(row) {
      ...
      if (row.comboDirectoryOid) {
        queryComboDirectoryMaterialList(row.comboDirectoryOid).then(
          response => {
    ...
    uploadSuccess(resp) {
      ...
      if (
        ...
        saveOrUpdateComboAhsSamplePicInfo(this.ahsSamplePicInfo).then(
          response => {
            if (response.code === 200) {
    ...
    deleteFile(row) {
      ...
      this.$confirm("是否确认删除?", "警告", {
        ...
        .then(() => {
          return deleteComboAhsSamplePicInfo(oid);
        })
        .then(() => {
          this.getaspiList(this.ahsSamplePicInfo);
    ...
    openExaminePointCardingView(item) {
      ...
      (this.fileViewurl =
        process.env.BASE_URL +
        "picture/prview.html?serviceOid=" +
        serviceOid +
        "&materiaOid=" +
        materiaOid +
        "&attaOid=" +
