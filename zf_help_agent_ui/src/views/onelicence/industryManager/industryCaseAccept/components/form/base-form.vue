...
<script>
import BaseFormJs from "@/mixins/onelicence/baseForm.js";
// import { getCaseSituationList, } from "@/api/onething/comboManager/comboAccept/updateComboCase";
import {getCaseSituationList} from "@/api/onelicence/industryManager/industryCaseAccept/updateIndustryCase";
import ApplyUserIndustryList from "../../applyUserIndustryList";
export default {
  components: {ApplyUserIndustryList},
  mixins: [BaseFormJs],
  props: ["isTempComponent", "caseOid", "pCegisterType"],
  created () {
    this.getSelectCertificateType();

    this.getDistrictTree()
      .then(() => {
        this.isTempComponent && this.queryQlCaseInfo(this.caseOid);
      })
      .catch(() => {
        this.isTempComponent && this.queryQlCaseInfo(this.caseOid);
      });
  ...
  methods: {
    ...
    queryQlCaseInfo (userCase) {
      if (!!userCase) {
        getCaseSituationList(userCase).then(response => {
          this.$nextTick(() => {
            this.ruleForm = response?.data?.comboCase || {};
            this.ruleForm._rqhbTime = this.ruleForm.rqhbTime;
            this.ruleForm.ifAccept = 1;
            this.changeDeliveryWay(this.ruleForm.resultDeliveryWay);
            this.changeProxyFlag(this.ruleForm.proxyFlag);
            this.ruleForm.credentialType = this.ruleForm.credentialType;
            this.getAddressTree(this.ruleForm.addresseeAddress);
            //this.getSelectCertificateType();
            if (this.isTempComponent) {
              this.getDistrictTreeNew('DISTRICT-'+this.ruleForm.bussVenueDistrictOid);

              // 缓存个人信息
              if (this.pCegisterType == '0') {
                localStorage.setItem('idCardInfo', JSON.stringify({
                  CNName: this.ruleForm.applyUserName,
                  carID: this.ruleForm.credentialNumber
                }))
              }
              ...
              if (this.pCegisterType == '1') {
                localStorage.setItem('idCardInfo', JSON.stringify({
                  legalUserName: this.ruleForm.applyUserName,
                  legalIdCard: this.ruleForm.credentialNumber
                }))
              }
            }
          })
