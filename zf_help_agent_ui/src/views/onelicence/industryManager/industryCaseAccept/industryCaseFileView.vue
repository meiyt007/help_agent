...
<script>
  import { getIndustryCaseAttaByAttaOid,getPdf } from "@/api/onelicence/industryManager/industryCaseAccept/industryCaseAtta";
  import pdf from 'vue-pdf'
  export default {
    components: {
      pdf
    },
    name: "IndustryCaseFileView",
    props:["attaOid"],
    data() {
      return {
        flag:true,
    ...
    methods: {
      ...
      getAttaNginxPath(){
        if(!this.attaNginxPath){
          if(this.attaOid){
            getIndustryCaseAttaByAttaOid(this.attaOid).then(response => {
