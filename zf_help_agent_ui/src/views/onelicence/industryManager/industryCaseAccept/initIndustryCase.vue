/** * 窗口受理管理 * @author: wangxl * @date: 2020-11-23 */
<template>
...
import {
  ...
  queryIndustryOptionTitleByValOid,
  getIndustryDireMaterialList,
  getCertificateType,
  saveIndustryCase,
  uploadCaseMaterialFile,
  saveCaseMaterialAtta,
  getIndustryCaseByOid,
  saveCaseAccpet,
  ...
  blockSituationOptions,
  pbpjSaveCaseInfo,
  pushPbpjInfo,
  getloginUser
} from "@/api/onelicence/industryManager/industryCaseAccept/initIndustryCase";
import { getOne } from "@/api/onelicence/sxpz/comboDirectory";
...
import { checkApplyUserInDishonest } from "@/api/onelicence/industryManager/industryCaseAccept/industryCaseRqbz";
import industryCaseFileView from "@/views/onelicence/industryManager/industryCaseAccept/industryCaseFileView";
import { intelligentPretrialmaterialPrePrial } from "@/api/onelicence/industryManager/industryCaseAccept/industryIntelligentPretrial";
import industryIntelligentPretrial from "@/views/onelicence/clzs/directoryManagement/industryIntelligentPretrial";
import applyUserIndustryList from "@/views/onelicence/industryManager/industryCaseAccept/applyUserIndustryList";
...
export default {
  ...
  components: {
    ...
    applyUserIndustryList,
    caseCharge,
    viewCharge,
    hardwareScan
  },
  name: "industryCaseInfo",
  //定义获取父类传过来值的格式
  ...
  methods: {
      ...
      if (count > 0) {
        if (index == 1) {
          ...
          if (bixuan) {
            ...
            blockSituationOptions(options, _that.comboDireOid).then(
              response => {
                if (response.data == null) {
                  ...
                  _that.msgError("情形选项选择不正确，请重新选择！");
                  return false;
                }
              }
            );
        ...
        if (index == 4) {
          _that.catalogList = [];
          _that.checkInDishonestUser(index);
        }
      } else {
        _that.i = index;
        if (index == 0) {
          ...
          if (titles) {
            ...
            titles.forEach(ti => {
              ...
              if (ti.fillFlag == "1") {
                ...
                for (let opt of comboOptionVals) {
                  if (opt.ifCheck == true) {
                    titleValues.chooseFlag =
                      titleValues.chooseFlag + "," + ti.titleOid + opt.valOid;
                  }
                }
    ...
    getSituationOpinionTitle(optionTitles) {
      if (optionTitles) {
        optionTitles.forEach((optionTitle, index) => {
          ...
          this.direSituationOptionTitles.push(titleValues);
          if (optionTitle.moreStatus == 0) {
            optionTitle.comboOptionVals.forEach(optVal => {
              if (optVal.defaultFlag == 1) {
                this.checkList[index] = optVal.valOid;
                this.changeTitle(optionTitle.titleOid, optVal.valOid);
              }
            });
          }
          if (optionTitle.moreStatus == 1) {
            optionTitle.comboOptionVals.forEach(optVal => {
              if (optVal.defaultFlag == 1) {
                this.checkBoxList.push(optVal.valOid);
              }
            });
          }
            ...
            for (let opt of comboOptionVals) {
              if (opt.ifCheck == true) {
                titleValues.chooseFlag =
                  titleValues.chooseFlag +
                  "," +
                  optionTitle.titleOid +
                  opt.valOid;
              }
            }
            this.direSituationOptionTitlesMust.push(titleValues);
          }
          if (this.direSituationOptionTitlesMust.length > 0) {
            for (let oneTitle of this.direSituationOptionTitlesMust) {
              oneTitle.comboOptionVals.forEach(optionVal => {
                if (optionVal.defaultFlag == 1) {
    ...
    getComboDireMaterialList() {
      ...
      getIndustryDireMaterialList(this.comboDireOid, allValOids).then(
        response => {
    ...
    checkPbpjInfo(index) {
      ...
      _that.$refs["ruleForm"].validate(valid => {
        if (valid) {
          /*//验证表单是否必填
            if(_that.sxSerForm){
              if(_that.sxSerForm.isFormFlag==1){//必填
      ...
      pushPbpjCheckLogin().then(response => {
        if (response.data) {
          ...
          _that
            ...
            .then(function() {
              regIndustryCaseInfo(ruleForm, _that.pbpjCaseOkUrl).then(
                response => {
                  if (response.data) {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          ...
          if (_that.industryCaseOid != "") {
            getIndustryCaseByOid(_that.industryCaseOid).then(response => {
              _that.id = response.data.id;
              _that.industryCaseOid = response.data.caseOid;
          ...
          _that.ruleForm.caseOid = _that.industryCaseOid;
          //this.ruleForm.caseStatus = this.caseStatus;
          _that.ruleForm.applyUserType = _that.cegisterType;
          _that.ruleForm.comboDireOid = _that.comboDireOid;
          _that.ruleForm.comboDireMaterials = _that.comboDireMaterials;
          console.log(
            "_that.comboDireMaterials=" +
              JSON.stringify(_that.comboDireMaterials)
          );
          //情形
          _that.ruleForm.situationOid = _that.comboDireSituationOid;
          //选项值
          _that.ruleForm.valList = _that.allCheckBoxList;

          /*  alert(this.ruleForm.addresseeAddress)*/
          //收件人地址
          if (typeof _that.ruleForm.addresseeAddress != "string") {
            let address = "[";
            _that.ruleForm.addresseeAddress.forEach(val => {
              address += '"' + val + '",';
            });
            if (_that.ruleForm.addresseeAddress.length > 0) {
              address = address.substring(0, address.lastIndexOf(","));
            }
            address += "]";
            _that.ruleForm.addresseeAddress = address;
          ...
          //业务管辖地
          if (typeof _that.ruleForm.bussVenueDistrictOid != "string") {
            let districtOid = [];
            let oids = "";
            districtOid = _that.ruleForm.bussVenueDistrictOid;
            districtOid.forEach(oid => {
              oids += oid + ",";
            });
            _that.ruleForm.bussVenueDistrictOid = oids;
          ...
          //检查表单是否有必须填写的
          let formName = "";
          _that.sxSerFormList.forEach(items => {
            if (items.isFormFlag == 1 && items.reportOid == undefined) {
              formName = items.formName;
            }
          });
          if (formName != "") {
            _that.msgError(formName + "必须填写表单！");
            return false;
          }
          console.log("this.ruleForm=" + JSON.stringify(_that.ruleForm));
          saveIndustryCase(_that.ruleForm).then(response => {
            if (response.data != "") {
              ...
              _that.industryCaseMaterials = response.data.industryCaseMaterials;
              if (
                _that.industryCaseMaterials &&
                _that.industryCaseMaterials.length > 0
              ) {
                _that.industryCaseMaterials.forEach(item => {
                  let bb = {};
                  bb.caseMaterialOid = item.caseMaterialOid;
                  bb.materialName = item.materialName;
                  bb.mustFlag = item.mustFlag;
                  _that.collectionTypeList.push(bb);
                  //拼接材料目录
                  _that.comboDireMaterials.forEach((ml, z) => {
                    if (ml.materialOid == item.materialOid) {
                      item.comboDireMaterial = ml;
                    }
                  });
                });
              }
              _that.comboDireSituationOid = response.data.comboDireSituationOid;
              _that.checkList = response.data.valList;
              _that.getDistrictTree("", response.data.bussVenueDistrictOid);
              _that.getAddressTree(_that.ruleForm.addresseeAddress);
              _that.saveCaseForm(); //保存办件表单信息
    ...
    clzs(index, count) {
      ...
      if (this.collectionTypeList) {
        ...
        if (mustFlag == true) {
          this.msgError(mustName + "的收取方式未选择");
          return false;
        }
        //遍历收取类型
        ...
        let dataForm = {};
        dataForm.caseOid = this.industryCaseOid;
    ...
    caseAccpet(flag) {
      ...
      if (_that.industryCaseOid != "") {
        ...
        _that.acceptForm.sqTime = _that.createDate
    ...
    chooseCollectionType(val, item, index) {
      ...
      if (this.industryCaseMaterials && this.industryCaseMaterials.length > 0) {
        this.industryCaseMaterials.forEach((industryMat, z) => {
          if (industryMat.caseMaterialOid == item.caseMaterialOid) {
            industryMat.attaList = [];
          }
        });
      }
      //删除附件和智审目录的关系
    ...
    changeTitle(titleChooseOid, optionValOid) {
      ...
      queryIndustryOptionTitleByValOid(this.comboDireOid, optionValOid).then(
        response => {
          //情形
          let titles = response.data;
    ...
    changeTitleBox(item, val) {
      ...
      _that.checkBoxList.forEach(box => {
        //多选框勾选显示与勾选的选项值的选项
        if (val == box) {
          queryIndustryOptionTitleByValOid(this.comboDireOid, box).then(
            response => {
              if (response.data != null) {
                //情形
                let titles = response.data;
          ...
          //多选框取消隐藏与勾选的选项值的选项
          queryIndustryOptionTitleByValOid(this.comboDireOid, val).then(
            response => {
              //情形
              let titles = response.data;
              if (titles && titles.length > 0) {
    ...
    closeIframe(reportOid) {
      if (reportOid) {
        this.sxSerFormList.forEach((serForm, index) => {
          if (this.indexForm == index) {
            serForm.reportOid = reportOid;
          }
        });
    ...
    saveCaseForm() {
      ...
      if (_that.sxSerFormList) {
        _that.sxSerFormList.forEach((items, i) => {
          ...
          formData.oid = _that.caseFormOids[i];
          saveOrUpdateCaseForm(formData).then(response => {
            if (response.data) {
              _that.caseFormOids[i] = response.data;
            }
          });
        });
      }
    ...
    //查看电子表单
    viewFormFilling(formDataId, index) {
      let _that = this;
