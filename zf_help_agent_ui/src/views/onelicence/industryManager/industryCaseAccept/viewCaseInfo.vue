    ...
    <el-tab-pane label="基本信息" name="info">
      ...
      <table cellspacing="0" cellpadding="0" border="0" class="zf-zc-table">
        ...
        <tr>
          ...
          <td>
            <b v-if="form.info.applyUserType == 0">申请人名称：</b
            ><b v-else>申请单位名称：</b>
          </td>
        ...
        <tr>
          ...
          <td>
            <b v-if="form.info.applyUserType == 0">申请人邮政编码：</b
            ><b v-else>申请单位邮政编码：</b>
          </td>
        ...
        <tr>
          <td>
            <b v-if="form.info.applyUserType == 0">申请人手机：</b
            ><b v-else>申请单位手机：</b>
          </td>
          <td>{{ form.info.applyUserPhone }}</td>
          <td>
            <b v-if="form.info.applyUserType == 0">申请人电话：</b
            ><b v-else>申请单位电话：</b>
          </td>
              ...
              >
                <div class="check-list">
                  ...
                  <span v-if="data.moreStatus != 3">
                    <span
                      v-for="item in data.caseSituatResultList"
                      :key="item.oid"
                    >
                      {{ item.optionName }}
                    </span>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              icon="iconfont zfsoft-chakan"
              @click="handleChildCaseInfo(scope.row)"
...
<!--              <span v-if="outInfo.outStatus == 1">已出库</span>-->
          ...
          <tr>
            ...
            <td colspan="3">
              <span
                v-for="(data, index) in outInfo.industryCaseMaterials"
                :key="index"
              >
    ...
    <el-dialog
      v-dialog-drag
      :visible.sync="childCaseOpenView"
      v-if="childCaseOpenView"
      @close="closeChildView"
      :title="title"
      width="1100px"
      height="700px"
      scrollbar
    ...
    <el-dialog
      ...
      append-to-body
    >
     <case-file-view-industry :attaOid="fileAttaOid" @father-click="closeCaseFileView"></case-file-view-industry>
    ...
    >
      <FormView
        ...
        :formNames="reportForm.formNames"
      />
    </el-dialog>
    <!-- 记录信息-->
    <el-dialog
...
import {getIndustryCaseFormViewByCaseOidAndDesignOid,queryIndustryFormInfoByCaseOid,getCaseSituationList} from '@/api/onelicence/industryManager/industryCaseAccept/updateIndustryCase.js'
...
import { downloadFile } from "@/api/onelicence/industryManager/industryCaseAccept/industryCaseAtta";
import {
  queryComboDirectoryResult,
  queryComboSignByMaps
} from "@/api/onething/fzgl/licenseSignCombo";
import { industrybatchOutList } from "@/api/onelicence/industryManager/industryCaseAccept/materialClassifiler.js";
import {queryIndustryDirectoryResult} from "@/api/onelicence/fzgl/licenseSignIndustry";
...
export default {
  ...
  components: { IframeUrlView,ViewCaseInfoByOneCase,caseFileViewIndustry},
  props: ["caseOid"],
  data() {
    return {
      activeName: "info",
  ...
  methods: {
    ...
    getOneCase() {
      getCaseSituationList(this.caseOidIndex).then(response => {
        ...
        //办件材料信息
        this.form.caseMaterials = response.data.comboCase.industryCaseMaterials;
        // this.getcaseFormInfo();
        //编辑格式化地址
        const selfArr = this.form.info.addresseeAddress.split(";");
    ...
    getSituationResult() {
      industrygetCaseSituationResultByCaseOid({ caseOid: this.caseOidIndex }).then(res => {
    ...
    //查询办件表单
    getCaseForm(caseOid) {
      queryIndustryFormInfoByCaseOid(caseOid).then(response => {
    ...
    async viewCaseFromInfo(row) {
      this.formNames="";
      await getIndustryCaseFormViewByCaseOidAndDesignOid(this.caseOid, row.designOid).then(response => {
        if(response.data){
    ...
    batchOutList(caseOidIndex) {
      industrybatchOutList(caseOidIndex).then(response => {
    ...
    queryComboDirectoryResult(caseOidIndex) {
      queryIndustryDirectoryResult(caseOidIndex).then(response => {
