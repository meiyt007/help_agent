...
<template>
  <div>
    <div>
      <h3><div class="zf-zc-table--title">整合证照</div></h3>
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="text"
              icon="iconfont zfsoft-xiugai"
              @click="openmodelTemplateManage(scope.row)"
              v-hasPermi="['combo:directory:update']"
              >套打模板配置</el-button
            >
            <el-button
              ...
              icon="iconfont
            zfsoft-chakan"
              @click="openmodelTemplateManageView(scope.row)"
              >预览模板</el-button
            >
      ...
      <br />
      <h3><div class="zf-zc-table--title">事项结果信息</div></h3>
      <el-table
        ref="multipleTable"
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              >证照目录配置</el-button
            >
            <!--            <el-button size="mini" type="text" icon="iconfont zfsoft-shanchu" @click="handleSxDelete(scope.row)"
              v-hasPermi="['combo:directory:delete']">删除</el-button>-->
          </template>
        </el-table-column>
      </el-table>
      <!-- 添加或修改信息对话框 -->
...
import {
  saveIndustryResult,
  queryIndustryResultList,
  delIndustryResult
} from "@/api/onelicence/industryManager/industryResult";
import {
...
import industryResult from "@/views/onelicence/industryManager/industryDirectory/industryResult";
export default {
  components: {
    industryResult,
    chooseResult,
  ...
  methods: {
    ...
    getDirectoryResultlList() {
      this.loading = true;
      queryIndustryResultList(this.comboDireOid).then(response => {
        this.industryResultList = response.data;
        this.loading = false;
      });
    },
    getSxResultList() {
    ...
    //查看模板编制页面
    openmodelTemplateManageView(row) {
      var faModelTemplateOid = "";
      let viewOrEdit = "view";
      let url =
        process.env.BASE_URL +
        "bc/indexResult.html?faModelTemplateOid=" +
        faModelTemplateOid +
        "&materialCatalogOid=" +
        row.resultOid +
        "&viewOrEdit=" +
    ...
    //编辑模板页面
    openmodelTemplateManage(row, faModelTemplateOid) {
      faModelTemplateOid = "";
      let viewOrEdit = "edit";
      let url =
        process.env.BASE_URL +
