...
<template>
  <div class="app-container">
   ...
   <!-- 一件事事项材料列表-->
    <el-dialog :close-on-click-modal="false" :title="title" :visible.sync="openInitTc" v-if="openInitTc" width="1020px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="0px" class="dialog-table">
        <el-table v-loading="loading" :data="form.serviceMaterList" border>
          ...
          <el-table-column label="材料类型" width="150" align="center" prop="materialType"  >
            <template slot-scope="scope">
...
<!--          </el-table-column>-->
    ...
    <el-dialog title="审查要点梳理编辑" :visible.sync="openAttaListView" width="1000px" append-to-body>
      <div class="app-container">
...
<script>
import { queryComboDirectoryMaterialList, page, getSxServiceOne, saveOrUpdate, getTaocanList, saveOrUpdateComboAhsSamplePicInfo, getComboAhsSamplePicInfoList, deleteComboAhsSamplePicInfo } from "@/api/onething/clzs/materialCheckKeyManage/comboMaterialCheckKeyManage.js";
  ...
  methods: {
    ...
    handleInitTc (row) {
      ...
      if (row.comboDirectoryOid) {
        queryComboDirectoryMaterialList(row.comboDirectoryOid).then(response => {
    ...
    uploadSuccess (resp) {
      ...
      if (type == 'jpg' || type == 'JPG' || type == 'jpeg' || type == 'JPEG' || type == 'png' || type == 'PNG') {
        ...
        saveOrUpdateComboAhsSamplePicInfo(this.ahsSamplePicInfo).then(response => {
          if (response.code === 200) {
