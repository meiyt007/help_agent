...
<script>
import {downloadSysAtta,queryComboAttaListByMaterialOid} from "@/api/zc/clzs/materialCheckKeyManage/materialCheckKeyManage.js";
...
export default {
  name: "Home",
  components: {pdf},
  props: ['sampleInfoOid','comboDirectoryOid','materiaOid','fileViewurl','caseOid','caseMaterialOid','attaOids','attaLists'],
  data() {
    return {
      labelPosition: "top",
      activeName: "first",
  ...
  methods: {
    getComboAttaListold(){
      ...
      queryComboAttaListByMaterialOid(materialOid,comboDireOid).then(response => {
        let comboCaseMaterialAttaList=response.data.data.attaLat;
    ...
    getComboAttaList() {
        this.loading = true;
        let atta={};
        let attaList=JSON.parse(this.attaLists);
        if(attaList && attaList.length>0){
          attaList.forEach((ite,j)=>{
            if(attaList){
              let sysAtta=ite;
                atta={};
