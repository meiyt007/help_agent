        ...
        <template v-for="(ruleForm, index) in form.faModelRuleValidationList">
          <template v-if="ruleForm.delFlag === 0">
            <tr>
              ...
              <td>
                <el-form-item :prop="'faModelRuleValidationList.' + index + '.basicForms'" :rules="rules.basicForms">
                  <el-select v-model="ruleForm.basicForms" placeholder="请选择基础表单元素" :clearable="true">
                    <el-option
                      v-for="mco in fieldTypeList"
                      :key="mco.oid"
                      :label="mco.fieldName"
                      :value="
                        mco.oid + ';' + mco.fieldKey + ';' + mco.fieldName
                      "
                    ></el-option>
                  </el-select>
                </el-form-item>
...
<script>
import { getBasicFieldListByType, queryFaModelRuleValidationList, saveOrUpdateFaModelRuleValidation, queryMaterialAndCataAndElementTree } from "@/api/onething/clzs/modelManagement/comboFaModelRuleValidation";
import { querySxFieldTypeAndLabelAndFieldTree } from "@/api/onething/comboForm/fieldManage";
export default {
  ...
  methods: {
    ...
    getSxFieldTypeAndLabelTree () {
      querySxFieldTypeAndLabelAndFieldTree(this.comboDirectoryOid).then(response => {
    ...
    //基础表单
    getBasicFieldListByType () {
      getBasicFieldListByType(this.JcbdqueryParams).then(response => {
        this.fieldTypeList = response.data;
        //console.log(JSON.stringify(this.fieldTypeList))
      });
    },
