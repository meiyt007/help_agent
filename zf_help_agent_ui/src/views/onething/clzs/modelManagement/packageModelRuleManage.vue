...
<template>
  <div class="el-table__header-wrapper dialog-table require-table">
    ...
    <table v-if="isFixedTop" :class="[isFixedTop ? 'fixed-to-top' : '']" cellspacing="0" cellpadding="0" border="0" class="zf-zc-table--common">
      <colgroup>
        <col width="3%" />
        <col width="8%" />
        <col width="17%" />
    ...
    <table cellspacing="0" cellpadding="0" border="0" class="zf-zc-table--common">
      <colgroup>
        <col width="3%" />
        <col width="8%" />
        <col width="17%" />
      ...
      <template v-for="(item, index) in serviceMaterList">
        <tr v-for="(materialCatalog, indexx) in item.materialCatalogList">
          <td
            ...
            v-show="indexx == 0"
            :rowspan="item.materialCatalogList.length"
          >
            {{ index + 1 }}
          </td>
          <td v-show="indexx == 0" :rowspan="item.materialCatalogList.length">
            {{ item.materialName }}({{ item.refinedMaterialName }})
          </td>
          <td>
            <div class="add-box">
              <span
            ...
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  ...
                  rule.delFlag === 0 &&
                  rule.sxMaterialOid === item.materialOid &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  item.oid == rule.refinedMaterialOid
                "
              >
...
export default {
  props: ['comboDirectoryOid', 'dialog'],
  data () {
    return {
      num: 0,
      selectData: [
  ...
  computed: {
    scrollbar () {
      return this.dialog?.[0]?.$refs?.scrollbar;
    }
  },
  created () {
    //初始化数据
  ...
  methods: {
    handleScroll () {
      let scrollbarEl = this.scrollbar.$refs.wrap;
      scrollbarEl.onscroll = () => {
        if (scrollbarEl.scrollTop > 146) {
    ...
    addYZ (index, indexx, indexmc, comboDirectoryOid, materialSampleName, catalogName, materialOid, materialCatalogOid, faModelTemplateBlockOid, blockName, blockCode
      , refinedMaterialOid, refinedMaterialName, serviceOid) {
      ...
      if (this.validateType != '') {
        ...
        if (this.validateType == this.VALIDATION_TYPE_CONTENT_THAN || this.validateType == this.VALIDATION_TYPE_SIMILARITY) {
          ...
          if (json == '{}') {
            ...
            this.faModelRuleValidation.comboDirectoryOid = this.comboDirectoryOid;
          ...
          packageModelRuleValidation.comboDirectoryOid = this.comboDirectoryOid;
          ...
          packageModelRuleValidation.serviceOid = serviceOid;
          if (this.validateType == this.VALIDATION_TYPE_CONTENT) {//文本验证
            ...
            sfczFlag = this.ruleCheck(this.VALIDATION_TYPE_NOTNULL, faModelTemplateBlockOid, comboDirectoryOid, materialCatalogOid, materialOid, refinedMaterialOid);
          } else if (this.validateType == this.VALIDATION_TYPE_NULL) {//为空
            sfczFlag = this.ruleCheck(this.VALIDATION_TYPE_NULL, faModelTemplateBlockOid, comboDirectoryOid, materialCatalogOid, materialOid, refinedMaterialOid);
          }
          if (sfczFlag != 'Y') {
    ...
    ruleCheck (ruleType, blockOid, comboDirectoryOid, materialCatalogOid, materialOid, refinedMaterialOid) {
      let flag = 'N';
      if (ruleType == this.VALIDATION_TYPE_NOTNULL) {
        for (var i = 0; i < this.faModelRuleValidationList.length; i++) {
          ...
          if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid && ruleInfo.validateType == this.VALIDATION_TYPE_NULL
            && ruleInfo.comboDirectoryOid == comboDirectoryOid && ruleInfo.catalogOid == materialCatalogOid && ruleInfo.sxMaterialOid == materialOid
            && ruleInfo.refinedMaterialOid == refinedMaterialOid) {
            this.$alert("当前区块已经配置【必须为空】规则，不能配置【不能为空】规则！");
            flag = 'Y';
          } else if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid && ruleInfo.validateType == this.VALIDATION_TYPE_NOTNULL
            && ruleInfo.comboDirectoryOid == comboDirectoryOid && ruleInfo.catalogOid == materialCatalogOid && ruleInfo.sxMaterialOid == materialOid
            && ruleInfo.refinedMaterialOid == refinedMaterialOid) {
      ...
      if (ruleType == this.VALIDATION_TYPE_NULL) {	//当前配置的规则：必须为空，必须为空不能存在非空和文本验证的规则
        for (var i = 0; i < this.faModelRuleValidationList.length; i++) {
          var ruleInfo = this.faModelRuleValidationList[i];
          if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid
            && (ruleInfo.catalogOid == materialCatalogOid && ruleInfo.sxMaterialOid == materialOid)
            && (ruleInfo.validateType == this.VALIDATION_TYPE_NOTNULL || ruleInfo.validateType == this.VALIDATION_TYPE_CONTENT)
            && ruleInfo.refinedMaterialOid == refinedMaterialOid) {
            this.$alert("当前区块已经配置【不能为空】或【文本验证】规则，不能配置【必须为空】规则！");
            flag = 'Y';
          } else if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid
            && ruleInfo.validateType == this.VALIDATION_TYPE_NULL
            && (ruleInfo.catalogOid == materialCatalogOid && ruleInfo.sxMaterialOid == materialOid)
            && ruleInfo.refinedMaterialOid == refinedMaterialOid) {
    ...
    getDateCompare () {
      /* if((null==this.contentDateFormat||this.contentDateFormat=='')||(null==this.contentDateValue||this.contentDateValue=='')){
         ...
         this.explain=" 输入日期格式和比对日期后，选择需要进行验证的区块。日期格式："+this.contentDateFormat+";比对日期："+this.contentDateValue;
         this.openDateCompare = false;
       }*/
