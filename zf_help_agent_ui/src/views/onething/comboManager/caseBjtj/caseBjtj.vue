  ...
  <div class="app-container">
    ...
    <el-table :data="caseRegList" v-loading="loading" border :fit="true">
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-show="scope.row.informStatus == '1'"
            size="mini"
            type="text"
      ...
      >
        ...
        >
          ...
          <el-table-column
            ...
            prop="materialName"
          />
          <el-table-column label="是否已收取" align="center">
            <template slot-scope="scope">
              <p v-if="scope.row.collectionFlag == 1">已收取</p>
              <p v-if="scope.row.collectionFlag == 0">未收取</p>
            </template>
    ...
    >
      ...
      >
        ...
        >
          ...
          <el-table-column label="收取方式" width="200" prop="collectionType">
            <template slot-scope="scope">
              <p v-if="scope.row.collectionType == 1">纸质收取</p>
              <p v-if="scope.row.collectionType == 2">附件上传</p>
              <p v-if="scope.row.collectionType == 3">扫描</p>
...
} from "@/api/onething/comboManager/comboAccept/caseBjtj.js";
import {
  getOneMaterialInfo
...
import { getComboCaseAttaByAttaOid } from "@/api/onething/comboManager/comboAccept/comboCaseAtta";
  ...
  methods: {
    ...
    handleInit (row) {
      ...
      if (row.caseOid) {
        ...
        console.log(row.legalPersonName);
        var legalPersonName = row.legalPersonName;
        if (legalPersonName) {
          this.getSelectCertificateType("1");
        } else {
    ...
    submitForm: function () {
      let _that = this;
      _that.form.applyPhone = this.caseInfo.applyUserPhone;
      _that.$refs["formEdit"].validate(valid => {
        if (valid) {
    ...
    submitInForm: function () {
      ...
      _that.$refs["formGz"].validate(valid => {
        if (valid) {
        ...
        if (valid) {
          /*queryCaseReturnByCaseOid(_that.caseOid).then(response => {
            if(response.data !=null){
              this.form=response.data;
              //图片预加载
              querySysAttaByOid(_that.form.imgInfo).then(response => {
                if(null!==response.data&&null!==response.data.fastdfsNginxUrl){
                  _that.verifyImg = response.data.fastdfsNginxUrl;
                }

              });
            }
            this.dialogTitleYL = "预览退件告知单";
            _that.issuedReceiveShow = true;
          });*/
          //图片预加载
          getComboCaseAttaByAttaOid(_that.form.imgInfo).then(response => {
            if (null !== response.data && null !== response.data.fastdfsNginxUrl) {
              _that.verifyImg = response.data.fastdfsNginxUrl;
            }
