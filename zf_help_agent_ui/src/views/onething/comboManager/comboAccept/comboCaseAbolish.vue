...
<template>
  <div class="app-container">
    <div class="process-box" id="print">
        ...
        >
              ...
              <td colspan="3">
                <el-form-item prop="acceptOpinionDesc">
                  <el-col :span="24">
                    <el-input
                      type="textarea"
                      v-model.trim="acceptForm.acceptOpinionDesc" name="acceptOpinionDesc"
                      maxlength="200"
                      show-word-limit
                    ></el-input>
                  </el-col>
          ...
          <div class="btn-wrap">
            <div class="btn-list mt10">
              ...
              <el-button type="primary" icon="el-icon-video-pause" @click="caseAccpet">受理</el-button>
            </div>
          </div>
        </el-form>
      </div>
      ...
      <!--表单嵌套页面-->
      <el-dialog title="电子表单" :visible.sync="iframeState" width="80%" height="100%" v-if="iframeState" :modal-append-to-body="true" append-to-body>
        <iframe-url :outLink="iframeUrl" @closeIframe="closeIframe"></iframe-url>
      </el-dialog>

      <el-dialog title="电子表单预览" :visible.sync="iframVieweState" width="80%" v-if="iframVieweState" :modal-append-to-body="true" append-to-body height="100%">
        <iframe-url-view :outLink="iframeUrlView" @closeIframeView="closeIframeView"></iframe-url-view>
      </el-dialog>
    </div>
    <!--</el-dialog>-->
    <!--引入文件的预览弹出层-->
    <el-dialog v-dialog-drag title="文件预览" :visible.sync="view.show" v-for="view in viewDialogOptions"  width="60%"  append-to-body>
      <combo-case-file-view :attaOid="view.attaOid"  @father-click="closeFileView"></combo-case-file-view>
    </el-dialog>
...
<script>
  import {getCaseSituationList,getCertificateType, saveComboCase, uploadCaseMaterialFile,saveCaseMaterialAtta
    ,saveCaseAccpet,pushPbpjCheckLogin,regComboCaseInfo} from "@/api/onething/comboManager/comboAccept/updateComboCase";
  ...
  import {validatePhone, validIDCard, validatePostCode, validateEmails} from '@/utils/validate';
  ...
  import {saveOrUpdateCaseForm, selectBySxSerForm,caseFormByCase} from "@/api/zc/businessManagement/formConfig";

  import comboCaseFileView from "@/views/onething/comboManager/comboAccept/comboCaseFileView";
  export default {
    inheritAttrs: false,
    components: {Treeselect, VueCropper,comboCaseFileView,packageMaterialComparison,IframeUrl,IframeUrlView},
    name: "updateCaseInfo",
    //定义获取父类传过来值的格式
    props:["caseOid"],
    data() {
      return {
        ...
        stepData:  [{index: "0", title: "情形选择",}, {index: "1", title: "材料核验",}, {index: "2", title: "信息登记",},
          {index: "3", title: "收取材料",}, {index: "4", title: "进入受理",},],
        selectData: [],
        // 表单校验
        rules: {
          projectName: [
    ...
    methods: {
      ...
      getComboCaseSituationList() {
        ...
        getCaseSituationList(this.caseOid).then(response => {
          ...
          this.comboDireOid = response.data.comboCase.comboDireOid;
          this.cegisterType = response.data.comboCase.applyUserType;
          //获取证照类型
          this.getSelectCertificateType();
          //获取一件事目录信息
          this.comboDireForm = response.data.comboDirectory;
          this.ruleForm = response.data.comboCase;
          //获取一件事材料
          this.comboDireMaterials = response.data.directoryMaterials;
          //办件信息
          this.id = response.data.comboCase.id;
          this.caseOid = response.data.comboCase.caseOid;
          this.caseNumber = response.data.comboCase.caseNumber;
          this.caseStatus = response.data.comboCase.caseStatus;
          //办件材料信息
          this.comboCaseMaterials = response.data.comboCase.comboCaseMaterials;
          this.$nextTick(() => {
            this.materialAttaList = [];
            this.comboCaseMaterials.forEach(mat => {
              let bb = new Object();
          ...
          this.changeDeliveryWay(response.data.comboCase.resultDeliveryWay);
          this.changeProxyFlag(response.data.comboCase.proxyFlag);
          this.getDistrictTree("",response.data.comboCase.bussVenueDistrictOid);
        });
      },
      /** 填充选项 */
      ...
      saveComboCaseForm(index) {
        this.$refs["ruleForm"].validate((valid) => {
          if (valid) {
            if(undefined != this.ruleForm.bussVenueDistrictOid && this.ruleForm.bussVenueDistrictOid.length > 0 ){
              let oid="";
              for (let i = 0; i < this.ruleForm.bussVenueDistrictOid.length; i++) {
        ...
        this.$nextTick(() => {
          this.comboCaseMaterials.forEach(ma => {
        ...
        pushPbpjCheckLogin().then(response => {
          if(response.data){
            ...
            this.$confirm('你确定要进行办件信息确认吗？', "警告", {
              ...
              regComboCaseInfo(ruleForm).then(response => {
                if(response.data == true){alert('dddd');
