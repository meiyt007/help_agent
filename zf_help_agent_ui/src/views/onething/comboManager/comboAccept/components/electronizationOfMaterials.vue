...
import {
  getElecMaterialsList,
  updateComboCaseMaterialList,
  saveComboCaseMaterialAttaList
} from "@/api/onething/comboManager/comboAccept/materialCategory.js";
import { queryElecLicenseByDirCode } from "@/api/onething/comboManager/comboAccept/elemLice.js";
import {
  getElecLicenUrl,
...
import caseFileView from "@/views/onething/comboManager/comboAccept/comboCaseFileView.vue";
...
export default {
  ...
  methods: {
    ...
    async nextStep () {
      ...
      if (fileUpLoadList.length !== 0) {
        ...
        this.materialAttaList = fileUpLoadList.map(item => {
          return {
            ...
            comboCaseMaterialAttaList: item.attaList.map(i => ({
              attaOid: i.attaOid,
              caseMaterialOid: item.caseMaterialOid,
        ...
        if (!hasScanning) {
          saveComboCaseMaterialAttaList(this.materialAttaList).then(res => {
      ...
      try {
        const { code, messge } = await updateComboCaseMaterialList(
