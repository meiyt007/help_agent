...
<script>
import IntelligentFormFilling from "@/mixins/onething/intelligentFormFilling";
...
import { getElecMaterialsList } from "@/api/onething/comboManager/comboAccept/materialCategory.js";
// 表单组件
...
export default {
  ...
  methods: {
    ...
    async getCaseSituationResultByCaseOid () {
      await getCaseSituationResultByCaseOid({ caseOid: this.caseOid }).then(
        ({ code, data }) => {
    ...
    //材料附件
    getAttaList () {
      getElecMaterialsList({ caseOid: this.caseOid }).then(({ code, data }) => {
        if (code !== 200 || !data)
