        ...
        <div class="step-content step-fourth" v-if="show_3">
          ...
          <el-form
            ...
            <table cellspacing="0" cellpadding="0" border="0" class="data-table">
              ...
              <tbody v-for="(item,index) in comboCaseMaterials">
              <tr>
                <td>{{ index + 1 }}</td>
...
<script>
  import {getCaseSituationList,getCertificateType} from "@/api/onething/comboManager/comboAccept/updateComboCase";
  ...
  import {caseFormByCase, selectBySxSerForm} from "@/api/zc/businessManagement/formConfig";
  ...
  export default {
    ...
    methods: {
      ...
      getSelectCertificateType() {
        if(this.cegisterType =="" || this.cegisterType ==undefined){
          this.cegisterType =0;
        }
      ...
      getComboCaseSituationList() {
        //查询事项情形
        getCaseSituationList(this.comboCaseOid).then(response => {
         // this.situationName = "默认自定情形";
          ...
          //编辑格式化地址
          var selfArr = this.ruleForm.addresseeAddress.split(";");
          /*var selfArr = '['+selfArr+']';*/
          if (null != selfArr) {
            var provinceCode = '';
            var zhixia = '';
      ...
      getcaseFormInfo(){
        ...
        selectBySxSerForm(this.comboDireOid).then(response => {
          if(response.data){
            ...
            //查询办件表单是否存在
            caseFormByCase(this.comboDireOid,this.comboCaseOid).then(response => {
              if(response.data){
