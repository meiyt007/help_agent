...
<script>
  import { getComboAttaByAttaOid,getPdf } from "@/api/onething/sxpz/comboAtta";
  import pdf from 'vue-pdf'
  export default {
    components: {
      pdf
    },
    name: "comboDireFileView",
    ...
    methods: {
      ...
      getAttaNginxPath(){
        if(!this.attaNginxPath){
          if(this.attaOid){
            console.log("this.attaOid==="+this.attaOid);
            getComboAttaByAttaOid(this.attaOid).then(response => {
      ...
      preview(ty,name,fileUrl) {
        ...
        if(1 === ty){
          ...
          this.fastFlag ='0';
          this.fileUrl = process.env.VUE_APP_BASE_API + "/combo/atta/downloadFile/"+ fileUrl;
        }
