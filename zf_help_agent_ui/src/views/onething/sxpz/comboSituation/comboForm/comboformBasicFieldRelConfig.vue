...
<template>
  <div>
    <el-row :gutter="10" class="mb8 fl">
      <el-col :span="1.5">
        <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleInit">新增</el-button>
      </el-col>
    </el-row>
    ...
    <el-table v-loading="loading" :data="labelList" border="">
      ...
      <el-table-column label="填充来源" align="center" prop="fillSource" :show-overflow-tooltip="true">
        <template scope="scope">
          <span v-if="scope.row.fillSource == 1">电子证照</span>
          <span v-if="scope.row.fillSource == 2">OCR识别结果</span>
        </template>
      </el-table-column>
      <el-table-column label="填充字段" align="center" prop="temFieldName" :show-overflow-tooltip="true" />
      <el-table-column label="基础表单字段" align="center" prop="fillFieldName" :show-overflow-tooltip="true" />
          ...
          <tr>
            ...
            <td>
              <el-form-item prop="tempFieldOid">
                <el-cascader v-if="form.fillSource == 1" v-model="form.tempFieldOid" :options="elecLicenList"
                  placeholder="电子证照元素" size="small" style="width: 240px" clearable filterable>
                </el-cascader>

                <el-cascader v-if="form.fillSource == 2" v-model="form.tempFieldOid" :options="ocrElementList"
                  placeholder="ocr识别结果元素" size="small" style="width: 240px" clearable filterable>
                </el-cascader>
              </el-form-item>
            </td>
            ...
            <td>
              <el-form-item prop="fillFieldOid">
                <el-select v-model.trim="form.fillFieldOid" placeholder="基础表单字段" size="small" style="width: 240px"
                  clearable filterable>
                  <el-option v-for="dept in basicFieldList" :key="dept.oid"
                    :label="dept.fieldName + '【' + dept.fieldKey + '】'" :value="dept.oid" />
                </el-select>
...
<script>
  ...
  import {
    ...
    checkHasRepeat
  } from "@/api/onething/comboForm/comboformFieldRelConfig.js";
  export default {
    components: {},
    props: ["comboDirectoryOid", "serviceName"],
    name: "comboformBasicFieldRelConfig",
    data() {
      return {
        // 遮罩层
        loading: true,
    ...
    methods: {
      ...
      submitForm: function () {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            checkHasRepeat(this.comboDirectoryOid, this.form.fillType, this.form.fillFieldOid, this.form.oid)
              .then(response => {
