/* * @Description:一件事情形材料关系配置列表 * @Author: dxl **/
<template>
  <div>
    ...
    <el-table v-loading="loading" :data="situationAttaRelList" :fit="true">
      ...
      <el-table-column label="材料" min-width="30%" align="center">
        <template slot-scope="scope">
          <div
            align="left"
            class="div-ellipsis"
            v-for="(item, idx) in scope.row.materialList"
            :key="idx"
...
<script>
import {
  optionMaterialVoPage,
  delMaterialRel
} from "@/api/onething/comboSituation/optionMaterial.js";
import situationAttaRelAdd from "@/views/onething/sxpz/comboSituation/situationAtta/situationAttaRelAdd.vue";
export default {
  components: {
    situationAttaRelAdd
  },
  ...
  name: "ComboSituationAttaRelIndex",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      optionMaterialVoPage(this.queryParams).then(response => {
        this.situationAttaRelList = response.data.data;
