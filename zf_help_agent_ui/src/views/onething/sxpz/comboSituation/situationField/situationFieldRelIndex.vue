/* * @Description:一件事情形字段配置列表 * @Author: dxl **/
<template>
  <div>
    <!-- <el-row :gutter="20"> -->
    <!--一件事目录列表数据-->
    ...
    <el-table v-loading="loading" :data="situationFieldRelList" :fit="true">
      ...
      >
        <template slot-scope="scope">
          <div
            align="left"
            v-for="(item, idx) in scope.row.fieldNames"
            :key="idx"
          >
            {{ item }}
          </div>
...
import situationFieldRelAdd from "@/views/onething/sxpz/comboSituation/situationField/situationFieldRelAdd.vue";
export default {
  components: {
    situationFieldRelAdd
  },
  props: {
    //一件事目录主键
    comboDirectoryOid: {
  ...
  name: "ComboSituationFieldRelIndex",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      page(this.queryParams).then(response => {
        this.situationFieldRelList = response.data.data;
