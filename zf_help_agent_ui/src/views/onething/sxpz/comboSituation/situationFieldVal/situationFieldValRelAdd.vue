      ...
      <div class="precondition">
        <div>
          <template v-for="(situ, idx) in situationList">
            <el-collapse :key="idx" v-model="situ.collapseFlag">
              <el-collapse-item
...
import {
  initFieldValRel,
  saveOptionFieldValRel,
  queryComboFieldList,
  queryComboFieldValList,
  getFormInfo
} from "@/api/onething/comboSituation/optionFieldVal.js";
export default {
  //定义获取父类传过来值的格式
  ...
  methods: {
    /** 查询情形列表 */
    handleInit () {
      initFieldValRel(this.comboDirectoryOid, this.relOid).then(response => {
        ...
        if (this.relOid != "") {
          ...
          this.fieldOids = oldOptRel.valOids.split(",");
          this.situationList.forEach(item => {
          ...
          this.queryComboFieldValList(this.form.fieldOid, 'init');
        }
      });
    },
    submitForm: function () {
      ...
      this.form.valOids = this.fieldOids.toString();
      this.form.optionOids = this.optionOids;
      this.form.controlType = this.controlType;
      saveOptionFieldValRel(this.form).then(response => {
    ...
    queryComboFieldList (val) {
      ...
      this.getFormInfo(this.comboDirectoryOid, val[0]);
    },
    // 查询表单编码
    getFormInfo (dirOid, typeOid) {
      getFormInfo(dirOid, typeOid).then(response => {
        this.formCode = response.data.formCode;
      });
    },
    // 查询字段下选项值
    queryComboFieldValList (fieldOid, type) {
      ...
      if (this.formCode == "") {
        getFormInfo(this.comboDirectoryOid, this.form.typeOid).then(
          response => {
            this.queryValList(this.small, response.data.formCode);
          }
        );
      } else {
    ...
    queryValList (fieldCode, formCode) {
      queryComboFieldValList(fieldCode, formCode).then(response => {
        if (
          response.data != null &&
          response.data.options != undefined &&
          response.data.options != null
        ) {
