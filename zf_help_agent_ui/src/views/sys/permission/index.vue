<template>
  <div class="app-container">
        ...
        <div class="head-container">
          ...
          >
            <span class="custom-tree-node" slot-scope="{ node, data }">
              ...
              <span v-if="node.label.length < 11" style="font-size: 14px">{{
                node.label
              }}</span>
            </span>
          </el-tree>
            ...
            >
              ...
              >
                ...
                <tr>
                  ...
                  <td>{{ form.info.appName }}</td>
                  <td>
                    <b>父级权限：</b>
                  </td>
                  <td>{{ form.info.parentName }}</td>
                </tr>
                <tr>
                  <td>
                    <i class="require">*</i>
                  ...
                  <td>
                    <el-form-item prop="functionType">
                      <el-radio-group
                        ...
                        v-model="form.info.functionType"
                      >
                ...
                <tr
                  id="tr1"
                  v-if="
                    form.info.isFrame == '0' && form.info.functionType == 'L'
                  "
                >
                  <td>
                    <i class="require">*</i>
                ...
                <tr
                  ...
                  v-if="
                    form.info.isFrame == '0' &&
                    form.info.functionType == 'L' &&
                    form.info.parentName != null
                  "
                >
                  <td>
                    <i class="require">*</i>
                  ...
                  <td colspan="3">
                    <el-col :span="12" style="width: 100%">
                      <el-form-item prop="hyperLink">
                        <el-input
                          placeholder="请输入访问路径"
                          v-model.trim="form.info.hyperLink"
                          maxlength="250"
                          show-word-limit
                        />
                      </el-form-item>
                ...
                <tr
                  id="tr3"
                  v-if="
                    form.info.isFrame == '0' && form.info.functionType == 'B'
                  "
                >
                  <td>
                    <b>访问路径：</b>
                  ...
                  <td colspan="3">
                    ...
                    <el-col :span="12" style="width: 100%">
                      <el-form-item prop="outLink" ref="outLinkForm">
                        <el-input
                          placeholder="请输入外链地址"
                          v-model.trim="form.info.outLink"
                          maxlength="200"
                          show-word-limit
                        />
                      </el-form-item>
                ...
                <tr>
                  <td>
                    <i class="require" v-if="form.info.functionType == 'B'"
                      >*</i
                    >
                    <b>权限标识：</b>
                  </td>
                  <td>
                    <el-form-item
                      prop="str"
                      key="key"
                      v-if="form.info.functionType == 'B'"
                    >
                      <el-input
                        placeholder="请输入权限标识"
                        v-model.trim="form.info.str"
                        maxlength="200"
                        show-word-limit
                      />
                    </el-form-item>
          ...
          <el-form ref="formLink" :model="form" :rules="links.rules">
            ...
            <el-table v-loading="loading" :data="form.links" border>
              <el-table-column label="链接地址" align="center" min-width="10%">
                <template slot-scope="scope">
                  <el-form-item
                    :prop="'links.' + scope.$index + '.perLink'"
                    :rules="links.rules.perLink"
                  >
                    <el-input
                      placeholder="请输入链接地址"
                      v-model.trim="scope.row.perLink"
                      maxlength="250"
                      show-word-limit
                    />
                  </el-form-item>
            ...
            <el-form-item style="padding-top: 40px; float: right">
              <el-button size="medium" type="primary" @click="saveLink"
                >确认</el-button
              >
    ...
    <el-dialog
      ...
      append-to-body
    >
    <div style="width:100%; min-height:300px; max-height:700px; overflow-y:scroll;" >
            ...
            <el-form :model="form.info" label-width="0px">
              ...
              >
                ...
                <tr>
                  ...
                  <td>{{ form.info.functionType == 'L' ? '菜单' : '按钮' }}</td>
                  <td>
                    <b>是否外链：</b>
                  </td>
                  <td>{{ form.info.isFrame == 1 ? '是' : '否' }}</td>
                </tr>
                <tr v-if="form.info.isFrame == 0">
                  <td>
                    <b>路由地址：</b>
                  </td>
                  <td>{{ form.info.router }}</td>
                  <td>
                    <b>访问路径：</b>
                  </td>
                  <td>{{ form.info.hyperLink }}</td>
                </tr>
                <tr v-if="form.info.isFrame == 1">
                  <td>
                    <b>外链地址：</b>
                  </td>
                  <td colspan="3">{{ form.info.outLink }}</td>
                </tr>
                <tr>
                  <td>
                    <b>权限标识：</b>
                  ...
                  <td>{{ form.info.sort }}</td>
                </tr>
                <tr>
                  <td>
                    <b>备注：</b>
                  </td>
                  <td colspan="3">{{ form.info.memo }}</td>
                </tr>
              </table>
            </el-form>
          </div>
        ...
        <el-tab-pane label="权限链接" name="link">
          <el-form :model="form">
            <el-table v-loading="loading" :data="form.links" border>
              <el-table-column label="链接地址" align="center" min-width="10%">
                <template slot-scope="scope">{{ scope.row.perLink }}</template>
              </el-table-column>
              <el-table-column label="备注" align="center" min-width="10%">
  ...
  data () {
    return {
      ...
      rules: {
        ...
        hyperLink1: [],
        str: [{ required: true, message: "权限标识不能为空", trigger: "blur" }],
        outLink: [
  ...
  methods: {
          ...
          save(this.form.info).then((res) => {
            if (res.code === 200) {
              ...
              if (permissionOid == undefined || null == permissionOid) {
                ...
                this.append(
                  node,
                  "PERMISSION-" + this.form.info.permissionOid,
                  this.form.info.name
                );
              } else {
                //更新一个节点
    ...
    handleDelete (row) {
      ...
      this.$confirm('是否确认删除"' + name + '"的数据项?', "警告", {
        ...
        .then(() => {
          ...
          //this.getTreeselect()
          if (
            "" != this.queryParams.parentOid &&
            "" != this.queryParams.appOid
          ) {
            const node = this.$refs.tree.getCurrentNode();
    ...
    isFrameChange (data) {
      ...
      if ("1" == data) {
        ...
        this.form.info.router = "iframe" + timestamp;
      } else {
        ...
        this.$refs["hyperLink1Form"] &&
          this.$refs["hyperLink1Form"].clearValidate();
      }
    ...
    defaultFrist (permissionTreeData) {
      ...
      for (let i = 0; i < permissionTreeData.length; i++) {
        if (
          "undefined" != typeof permissionTreeData[i].children &&
          permissionTreeData[i].children.length > 0
        ) {
