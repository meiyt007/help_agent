/**
* @Author: liangss
* @Date: 2021-04-21 14:35:16
* @Description: 一业一证办事人统计
*/
<template>
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="aplyUserList" border :fit="true">
...
<script>
import { getIndustryApplyUserStatistics, listExpIndustryApplyUserStatistics } from "@/api/tjfx/industryStatistics";
...
export default {
  name: "industryApplyUserStatistics",
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      getIndustryApplyUserStatistics(this.queryParams).then(response => {
        /* alert(JSON.stringify(response.data))*/
        this.aplyUserList = response.data.data;
    ...
    getzeroCaseComboDirect (row) {
      let item = { show: true, districtOid: row.districtOid, organOid: row.organOid, startDate: row.startDate, endDate: row.endDate, industryType: '', };
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出列表数据项?', "警告", {
        ...
        listExpIndustryApplyUserStatistics(queryParams);
      }).catch(function () { });
    }
