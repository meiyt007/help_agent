/**
* @Author: liangss
* @Date: 2021-04-21 15:18:16
* @Description: 一业一证目录统计
*/
<template>
  <div class="app-container">
        ...
        >
          ...
          <el-table-column label="办件数" align="center">
            <template slot-scope="scope">
              <a
                ...
                @click="getIndustryCaseList(scope.row, '')"
                >{{ scope.row.caseNum }}</a
              >
            </template>
          </el-table-column>
          <el-table-column label="受理数" align="center">
            <template slot-scope="scope">
              <a
                href="javascript:void(0);"
                @click="getIndustryCaseList(scope.row, 'SL')"
                >{{ scope.row.slMum }}</a
              >
            </template>
          </el-table-column>
          <el-table-column label="办结数" align="center">
            <template slot-scope="scope">
              <a
                href="javascript:void(0);"
                @click="getIndustryCaseList(scope.row, 'BJ')"
                >{{ scope.row.bjNUm }}</a
              >
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button size="mini" type="text" icon="iconfont zfsoft-chakan" @click="getIndustryCaseList(scope.row)" v-hasPermi="['sys:AreaSite:view']" >查看</el-button>
            </template>
          </el-table-column>-->
        </el-table>
    ...
    >
      <el-scrollbar style="height: 500px">
        <industry-case-list
          :comboDireOid="industry.comboDireOid"
          :caseType="industry.caseType"
          :themeOid="industry.themeOid"
          :startDate="industry.startDate"
          :endDate="industry.endDate"
        ></industry-case-list>
      </el-scrollbar>
    </el-dialog>
    <!-- 办件列表end-->
...
<script>
import { getIndustryComboDirectoryStatistics, listExpIndustryComboDirectoryStatistics } from "@/api/tjfx/industryStatistics";
...
import industryCaseList from "@/views/tjfx/industry/industryCaseList";
export default {
  name: "industryComboDirectoryStatistics",
  components: { Treeselect, industryCaseList },
  data () {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      getIndustryComboDirectoryStatistics(this.queryParams).then(response => {
    ...
    //调整办件列表页面
    getIndustryCaseList (row, caseType) {
      let title = "办件列表";
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出列表数据项?', "警告", {
        ...
        listExpIndustryComboDirectoryStatistics(queryParams);
      }).catch(function () { });
    }
  },
};
