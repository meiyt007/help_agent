/**
* @Author: liangss
* @Date: 2021-04-20 13:15:16
* @Description: 一件事零办件统计
*/
<template>
      ...
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="zeroCaseList" border :fit="true">
          ...
          <!--          <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button size="mini" type="text" icon="iconfont zfsoft-chakan" @click="getzeroCaseComboDirect(scope.row)" v-hasPermi="['sys:AreaSite:view']" >查看</el-button>
            </template>
          </el-table-column>-->
        </el-table>
    ...
    >
      <el-scrollbar style="height: 500px">
        <zero-case-combo-directory
          ...
          :industryType="zeroCase.industryType"
        ></zero-case-combo-directory>
      </el-scrollbar>
    </el-dialog>
    <!-- 零办件一件事目录列表end-->
...
<script>
import { getOnethingZeroCaseStatistics, listExpOnethingZeroCaseStatistics } from "@/api/tjfx/oneThingStatistics";
import { queryDistrictSimpleTree } from "@/api/sys/district";
import { queryOrganTree } from "@/api/sys/organ";
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";
import zeroCaseComboDirectory from "@/views/tjfx/onething/zeroCaseComboDirectory";
export default {
  name: "onethingZeroCaseStatistics",
  ...
  methods: {
    ...
    getList () {
      ...
      /*
            ...
            return;*/
      getOnethingZeroCaseStatistics(this.queryParams).then(response => {
    ...
    getzeroCaseComboDirect (row) {
      let title = "零办件目录列表";
      //let item = {show:true,districtOid:row.districtOid,organOid:row.organOid,startDate:row.startDate,endDate:row.endDate,industryType:'',title:title};
      ...
      let item = { show: true, districtOid: districtOid, organOid: organOid, startDate: startDate, endDate: endDate, industryType: '', title: title };
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出列表数据项?', "警告", {
        ...
        listExpOnethingZeroCaseStatistics(queryParams);
      }).catch(function () { });
    }
