...
<script>
import { queryZeroCaseComboDirectoryWithPage } from "@/api/tjfx/oneThingStatistics";
...
export default {
  ...
  props: ["districtOid", "organOid", "startDate", "endDate", "industryType"],
  data () {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      queryZeroCaseComboDirectoryWithPage(this.queryParams).then(response => {
