...
<script>
import { qlCaseImmediateRateCaseList } from "@/api/tjfx/qlCase/qlCaseStatistics";
import viewCaseInfo from "@/views/zc/businessManagement/windowAcceptance/viewCaseInfo";
export default {
  name: "qlCaseImmediateRateCaseList",
  components: { viewCaseInfo },
  props: ["viewType", "startDate", "endDate", "organOid"],
  data () {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      qlCaseImmediateRateCaseList(this.queryParams).then(response => {
