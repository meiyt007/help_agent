    ...
    <el-container :style="'height:'+fullHeight+'px;'">
      ...
      <el-main style="border: 1px solid #DCDFE6;background-color:#FAFAFA">
          <el-form label-width="auto" size="mini" label-position="top">
            <!-- 动态显示属性面板 -->
            <component :is= "propsComponent" :element= "element" :flowName="params.name" :key= "key"></component>
          </el-form>
      </el-main>
    </el-container>
...
export default {
  ...
  methods: {
      ...
      setBackColor() {
        ...
        const sequenceFlowList = elementRegistry.filter(reg => reg.type == "bpmn:SequenceFlow");
        const bhSequenceFlowList = sequenceFlowList.filter(reg => reg.businessObject.$attrs.flowClass == "bh");
        if(bhSequenceFlowList.length>0){
          bhSequenceFlowList.forEach(key => {
            canvas.addMarker(key.id, "highlight-bh");
          });
        }
        const thSequenceFlowList = sequenceFlowList.filter(reg => reg.businessObject.$attrs.flowClass == "th");
