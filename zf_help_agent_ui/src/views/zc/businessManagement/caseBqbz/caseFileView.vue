...
<script>
import { getCaseAttaByAttaOid, getPdf } from "@/api/zc/businessManagement/caseAtta";
  ...
  methods: {
    ...
    getAttaNginxPath () {
      if (!this.attaNginxPath) {
        if (this.attaOid) {
          getCaseAttaByAttaOid(this.attaOid).then(response => {
    ...
    preview (ty, name, fileUrl) {
      ...
      if (1 === ty) {
        ...
        this.fastFlag = '0';
        this.fileUrl = process.env.VUE_APP_BJFW_ROUTE_PATH + "/sysAttaService/download?attaOid=" + fileUrl;
      }
