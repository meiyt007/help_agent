...
<script>
import {downloadSysAtta, querySysAttaListByMaterialOid,queryComboAttaListByMaterialOid} from "@/api/zc/businessManagement/materialCheckKeyManage.js";
...
export default {
  name: "Home",
  components: {pdf},
  props: ['sampleInfoOid','comboDirectoryOid','materiaOid','fileViewurl','caseOid','caseMaterialOid'],
  data() {
    return {
      labelPosition: "top",
      activeName: "first",
  ...
  created() {
      this.getComboAttaList();
    },
  methods: {
    getComboAttaList(){
      this.loading = true;
      let atta={};
      //let materialOid="a1ae5328b47f4d3d89d6f14a88ff5d71";//550f0daf2b064c81825d737c6cc9d6cb
      let materialOid=this.materiaOid;
      let comboDireOid=this.comboDirectoryOid;
      let caseMaterialOid=this.caseMaterialOid;
      queryComboAttaListByMaterialOid(caseMaterialOid,comboDireOid).then(response => {
        let comboCaseMaterialAttaList=response.data.comboCaseMaterialAttaList;
        alert(comboCaseMaterialAttaList)
        let j=0;
        for (let i=0;i<comboCaseMaterialAttaList.length;i++){
          let comboAtta=comboCaseMaterialAttaList[i].comboCaseAtta;
          atta={};
          let type=this.iconByType(comboAtta.originName);
          ...
          if(type=='jpg'||type=='JPG'||type=='jpeg'||type=='JPEG'||type=='png'||type=='PNG'||type=='PDF'||type=='pdf'){
            ...
            atta.pageRotate=0;
            atta.pageTotalNum=1;
            atta.name=comboAtta.name;
            atta.url=comboAtta.fastdfsNginxUrl;
            this.attaList.push(atta);
          }
         //console.log(JSON.stringify(this.attaList))
        }
      })
    },
