/** * @Author: dxl * @Date: 2020-11-5 * @Description: 材料出库 */
<template>
...
<!--            <template slot="append"
    ...
    <pagination
      ...
      @pagination="getRegSxServiceOids"
    />
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <!-- <el-button type="primary" @click="openRobot"
            >机器人材料出库</el-button
          > -->
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="updateMaterialOUt"
          >更改出库材料清单</el-button
        >
        <!--  <el-button id="printObjLz" type="primary" v-print="printObjLz"    @click="printRecord">打印</el-button>-->
        <el-button id="printObjLz" type="primary" v-print="printObjLz"
          >打印</el-button
        >
...
import {
  ...
  printOutMaterial,
  getOpenRobot,
  updateCaseMaterialOutOfStock,
  saveCaseMaterialOutOfStockRecordByPrint
} from "@/api/zc/businessManagement/materialOut.js";
...
export default {
  ...
  data() {
    return {
      ...
      selectMaterialoids: [],
      honorCheckList: [],
      caseMaterialOutOfStockRecordList: [], //材料流转记录
      id: "",
      MATERIAL_OUT_URL: "",
  ...
  methods: {
    ...
    printRecord() {
      ...
      saveCaseMaterialOutOfStockRecordByPrint(_that.id).then(response => {
    ...
    confirmMaterialOUt() {
      ...
      updateCaseMaterialOutOfStock(this.caseMaterialOutOfStock).then(
        response => {
    ...
    confirmMaterialMenu(id) {
      this.id = id;
      getOneMaterialInfo(id).then(response => {
        this.caseMaterialList = response.data;
        ...
        printOutMaterial(id).then(response => {
          if (response.code === 200) {
            ...
            if (this.printCaseInfo.length > 0) {
              // this.printCaseInfo[0].appUrl = process.env.VUE_APP_OUT_API_PAGE + "?id=" + this.printCaseInfo[0].id + "&type=dbsx";
              this.printCaseInfo[0].appUrl =
                this.MATERIAL_OUT_URL +
                "?id=" +
                this.printCaseInfo[0].id +
                "&type=dbsx";
              console.log(this.printCaseInfo[0].appUrl);
            }
            this.$nextTick(() => {
    ...
    submitForm() {
      ...
      //this.form.materialOids=this.materialOidsArr.join(';');
    ...
    submitBatchForm() {
      ...
      if (this.multipleSelection.length <= 0) {
        this.$message.error("请选择出库数据!");
        return false;
      }
      for (let ss = 0; ss < this.multipleSelection.length; ss++) {
        id.push(this.multipleSelection[ss].id);
      }
      let allId = id.join(";");
