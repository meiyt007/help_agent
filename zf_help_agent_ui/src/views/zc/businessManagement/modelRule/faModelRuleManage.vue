...
*/
<template>
  <div class="fa-model-rule-manage">
    <div class="zf-zc-table--title">验证规则配置</div>
    ...
    <div class="require-box">
      <i class="el-icon-warning-outline"></i>{{ this.explain }}
    </div>
    <el-table
      :data="serviceMaterList"
      border
      style="width: 100%"
      height="500px"
    >
      ...
      >
        <template slot-scope="{ row }">
          {{ row.materialName }}({{ row.refinedMaterialName }})
        </template>
      </el-table-column>
      <el-table-column
        label="目录子项名称（目标编码）"
      ...
      >
        <template slot-scope="{ row, $index }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <div class="add-box">
              <span
                >{{ materialCatalog.catalogName }}
                <template
                  v-if="
                    materialCatalog.catalogCode != '' &&
                    materialCatalog.catalogCode != null
                  "
                >
                  ({{ materialCatalog.catalogCode }})
                </template>
              ...
              <ul
                v-for="(
                  faModelTemplateBlock, indexmc
                ) in materialCatalog.faModelTemplateBlockList"
                :key="indexmc"
              >
                <li
                  @click="
                    addYZ(
                      ...
                      materialCatalog.catalogName,
                      row.materialOid,
                      materialCatalog.materialCatalogOid,
                      faModelTemplateBlock.faModelTemplateBlockOid,
                      faModelTemplateBlock.blockName,
                      faModelTemplateBlock.blockCode,
                      row.oid,
                      row.refinedMaterialName
                    )
                  "
                >
                  <span>
                    ...
                    <template
                      v-if="
                        faModelTemplateBlock.blockCode != '' &&
                        faModelTemplateBlock.blockCode != null
                      "
                    >
                      ({{ faModelTemplateBlock.blockCode }})
                    </template>
      ...
      <el-table-column label="不能为空" width="240" align="center">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_NOTNULL &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
                "
              >
                <div
                  class="require-item"
      ...
      <el-table-column label="必须为空" width="240" align="center">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_NULL &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
      ...
      <el-table-column width="240" align="center" label="文本验证">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_CONTENT &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
      ...
      <el-table-column width="240" align="center" label="文本包含">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_STR_CONTAIN &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
      ...
      <el-table-column width="240" align="center" label="数字范围">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_RANGE &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
      ...
      <el-table-column width="240" align="center" label="日期比较">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_DATE_COMPARE &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
              ...
              >
                ...
                >
                  ● {{ rule.templateMetadataName }}({{
                    rule.templateMetadataCode
                  }}) 日期格式：{{ rule.contentDateFormat }};
                  <template
                    v-if="
                      rule.contentBeforeDateValue != '' &&
                      rule.contentBeforeDateValue != null
                    "
                  >
                    时间小于{{ rule.contentBeforeDateValue }};
                  </template>
                  <template
                    v-if="
                      rule.contentAfterDateValue != '' &&
                      rule.contentAfterDateValue != null
                    "
                  >
                    时间大于{{ rule.contentAfterDateValue }};
                  </template>
                  <template
                    v-if="
                      rule.noMoreThanAge != '' && rule.noMoreThanAge != null
                    "
                  >
                    年龄不超过{{ rule.noMoreThanAge }};
                  </template>
                  <template
                    v-if="
                      rule.noLessThanAge != '' && rule.noLessThanAge != null
                    "
                  >
                    年龄不小于{{ rule.noLessThanAge }};
                  </template>
                  <!--                      比对日期：{{rule.contentDateValue }}-->
                  <el-button
                    type="default"
                    icon="el-icon-error"
                    class="btn-close"
      ...
      <el-table-column width="240" align="center" label="手写验证">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_HAND_SIGN &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
      ...
      <el-table-column width="240" align="center" label="盖章验证">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_SEAL &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
      ...
      <el-table-column width="240" align="center" label="人工审核">
        <template slot-scope="{ row }">
          <tr
            v-for="(materialCatalog, indexx) in row.materialCatalogList"
            :key="indexx"
          >
            <template v-for="(rule, indexmc) in faModelRuleValidationList">
              <template
                v-if="
                  rule.validateType === VALIDATION_TYPE_MANUAL_AUDIT &&
                  rule.delFlag === 0 &&
                  rule.catalogOid === materialCatalog.materialCatalogOid &&
                  rule.sxMaterialOid == row.materialOid &&
                  row.oid == rule.refinedMaterialOid
    ...
    <div class="mt10 set-table">
      <div class="zf-zc-table--title">关联验证规则</div>
      <table
        cellspacing="0"
        cellpadding="0"
        border="0"
      ...
      >
        ...
        <template v-for="(rule, indexmc) in faModelRuleValidationList">
          <template
            v-if="
              (rule.validateType === VALIDATION_TYPE_CONTENT_THAN ||
                rule.validateType === VALIDATION_TYPE_SIMILARITY) &&
              rule.delFlag === 0
            "
          >
            <tr>
              <td>
                <template
                  v-if="rule.validateType === VALIDATION_TYPE_CONTENT_THAN"
                >
                  关联对比
                </template>
                <template
                  v-if="rule.validateType === VALIDATION_TYPE_SIMILARITY"
                >
                  相识度(>={{ rule.similar }}%)
                </template>
              </td>
              ...
              <td>
                {{ rule.thanTemplateMetadataName }}({{
                  rule.thanTemplateMetadataCode
                }})
              </td>
              <td>
                <el-button
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
      :title="title"
      :visible.sync="openDateCompare"
      v-if="openDateCompare"
      width="800px"
      append-to-body
    >
      <el-form ref="form" label-width="0px">
        <table cellspacing="0" cellpadding="0" border="0" class="zf-zc-table">
          ...
          <tr>
            ...
            <td colspan="3">
              <el-form-item prop="contentDateFormat">
                <el-col :span="24">
                  <el-input
                    v-model.trim="contentDateFormat"
                    placeholder="请输入日期格式"
                    maxlength="50"
                    show-word-limit
                  />
              ...
              <i class="require">*注:日期格式、小于日期、大于日期需和比较日期的数据格式一致（例如）</i></b>
              <i class="require">比较日期：2021年7月12日 日期格式：yyyy年MM月dd日;</i></b>
              <i class="require">比较日期：2021-7-12 日期格式：yyyy-MM-dd;</i></b>
              <i class="require">比较日期：20210712 日期格式：yyyyMMdd;</i></b>
            </td>
          </tr>
          <!--          <tr>
            <td>
              <b><i class="require">*</i>比较日期（当值为CURRENT_DATE时，与实时时间比对）(小于)：</b>
            </td>
            <td  colspan="3">
          ...
          <tr>
            <td>
              <b>小于日期（当值为CURRENT_DATE时，与实时时间比对）：</b>
            </td>
            <td colspan="3">
              <el-form-item prop="contentBeforeDateValue">
                <el-col :span="24">
                  <el-input
                    v-model.trim="contentBeforeDateValue"
                    placeholder="请输入比较日期"
                    maxlength="50"
                    show-word-limit
                  />
          ...
          <tr>
            <td>
              <b>大于日期（当值为CURRENT_DATE时，与实时时间比对）：</b>
            </td>
            <td colspan="3">
              <el-form-item prop="contentAfterDateValue">
                <el-col :span="24">
                  <el-input
                    v-model.trim="contentAfterDateValue"
                    placeholder="请输入比较日期"
                    maxlength="50"
                    show-word-limit
                  />
          ...
          <tr>
            ...
            <td colspan="3">
              <el-form-item prop="noMoreThanAge">
                <el-col :span="24">
                  <el-input
                    v-model.trim="noMoreThanAge"
                    placeholder="请输入年龄不超过数"
                    maxlength="10"
                    show-word-limit
                  />
                </el-col>
          ...
          <!--          <tr>
            ...
            <td  colspan="3">
              <el-form-item prop="biforeNowDayValue">
                <el-col :span="24">
                  <el-input v-model.trim="biforeNowDayValue" placeholder="请输入天数" maxlength="50" show-word-limit />
                </el-col>
              </el-form-item>
            </td>
          </tr>
          <tr>
            ...
            <td  colspan="3">
              <el-form-item prop="afterNowDayValue">
                <el-col :span="24">
                  <el-input v-model.trim="afterNowDayValue" placeholder="请输入天数" maxlength="50" show-word-limit />
                </el-col>
              </el-form-item>
            </td>
          </tr>-->
          ...
          <tr>
            ...
            <td colspan="3">
              <el-form-item prop="validSimilarity">
                <el-col :span="24">
                  <el-input
                    v-model.trim="validSimilarity"
                    placeholder="请输入相识度,相识度只能是两位正整数"
                    maxlength="2"
                    show-word-limit
                  />
                </el-col>
...
import { initFaModelRule, initAll, saveOrUpdate } from "@/api/zc/businessManagement/faModelRuleValidation.js";
export default {
  props: ['serviceOid'],
  data () {
    return {
      ...
      contentDateValue: "",
      //小于时间
      contentBeforeDateValue: "",
      //大于时间
      contentAfterDateValue: "",
      noMoreThanAge: "",
      noLessThanAge: "",
      //距离今天时间小于天数
      biforeNowDayValue: "",
  ...
  methods: {
    ...
    selectChange (index) {
      ...
      if (index == 0) {
        ...
        this.faModelRuleValidation = {};//重新点比对的时候清空验证规则
      }/*else if(index==8){
        this.openSimilarity = true;
        this.validateType=this.VALIDATION_TYPE_SIMILARITY;
        this.faModelRuleValidation={};//重新点比对的时候清空验证规则
      }*/else if (index == 8) {
        this.explain = "请点击目录子项中的区块名称，选择盖章的区块。";
        this.validateType = this.VALIDATION_TYPE_SEAL;
      } else if (index == 9) {
        this.explain = "请点击目录子项中的区块名称，选择人工审核的区块。";
        this.validateType = this.VALIDATION_TYPE_MANUAL_AUDIT;
      }
    ...
    cancelDateCompare () {
      this.contentBeforeDateValue = "";
      this.contentAfterDateValue = "";
      this.noMoreThanAge = "";
      this.explain = "已选择验证类型：";
      this.validateType = "";
      this.num = 111;
    ...
    initInfo () {
      ...
      initFaModelRule(this.queryParams).then(response => {
        ...
        /* console.log(JSON.stringify(this.serviceMaterList))*/
    ...
    //添加验证
    addYZ (index, indexx, indexmc, serviceOid, materialName, catalogName, materialOid, materialCatalogOid, faModelTemplateBlockOid, blockName, blockCode, refinedMaterialOid, refinedMaterialName) {
      let sfczFlag = 'N';
      ...
      if (this.validateType != '') {
        ...
        if (this.validateType == this.VALIDATION_TYPE_CONTENT_THAN || this.validateType == this.VALIDATION_TYPE_SIMILARITY) {
          ...
          if (json == '{}') {
            ...
            this.faModelRuleValidation.serviceOid = this.serviceOid;
            ...
            this.faModelRuleValidation.sxMaterialName = materialName;
            ...
            this.faModelRuleValidation.refinedMaterialOid = refinedMaterialOid;
            this.faModelRuleValidation.refinedMaterialName = refinedMaterialName;
            ...
            this.faModelRuleValidation.thanSxMaterialName = materialName;
            ...
            this.faModelRuleValidation.thanRefinedMaterialOid = refinedMaterialOid;
            this.faModelRuleValidation.thanRefinedMaterialName = refinedMaterialName;
          ...
          packageModelRuleValidation.serviceOid = this.serviceOid;
          ...
          packageModelRuleValidation.sxMaterialName = materialName;
          ...
          packageModelRuleValidation.refinedMaterialOid = refinedMaterialOid;
          packageModelRuleValidation.refinedMaterialName = refinedMaterialName;
          ...
          if (this.validateType == this.VALIDATION_TYPE_CONTENT) {//文本验证
            ...
            packageModelRuleValidation.contentBeforeDateValue = this.contentBeforeDateValue;
            packageModelRuleValidation.contentAfterDateValue = this.contentAfterDateValue;
            packageModelRuleValidation.biforeNowDayValue = this.biforeNowDayValue;
            packageModelRuleValidation.afterNowDayValue = this.afterNowDayValue;
            packageModelRuleValidation.noMoreThanAge = this.noMoreThanAge;
            packageModelRuleValidation.noLessThanAge = this.noLessThanAge;
          } else if (this.validateType == this.VALIDATION_TYPE_NOTNULL) {//不为空
            sfczFlag = this.ruleCheck(this.VALIDATION_TYPE_NOTNULL, faModelTemplateBlockOid, refinedMaterialOid);
          } else if (this.validateType == this.VALIDATION_TYPE_NULL) {//为空
            sfczFlag = this.ruleCheck(this.VALIDATION_TYPE_NULL, faModelTemplateBlockOid, refinedMaterialOid);
          }
          if (sfczFlag != 'Y') {
    ...
    ruleCheck (ruleType, blockOid, materialOid) {
      let flag = 'N';
      if (ruleType == this.VALIDATION_TYPE_NOTNULL) {
        for (var i = 0; i < this.faModelRuleValidationList.length; i++) {
          ...
          if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid && ruleInfo.validateType == this.VALIDATION_TYPE_NULL && materialOid == ruleInfo.sxMaterialOid) {
            this.$alert("当前区块已经配置【必须为空】规则，不能配置【不能为空】规则！");
            flag = 'Y';
          } else if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid && ruleInfo.validateType == this.VALIDATION_TYPE_NOTNULL && materialOid == ruleInfo.sxMaterialOid) {
            flag = 'Y';
          }
        }
      }
      if (ruleType == this.VALIDATION_TYPE_NULL) {	//当前配置的规则：必须为空，必须为空不能存在非空和文本验证的规则
        for (var i = 0; i < this.faModelRuleValidationList.length; i++) {
          var ruleInfo = this.faModelRuleValidationList[i];
          if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid && (ruleInfo.validateType == this.VALIDATION_TYPE_NOTNULL || ruleInfo.validateType == this.VALIDATION_TYPE_CONTENT) && materialOid == ruleInfo.sxMaterialOid) {
            this.$alert("当前区块已经配置【不能为空】或【文本验证】规则，不能配置【必须为空】规则！");
            flag = 'Y';
          } else if (ruleInfo.delFlag == 0 && ruleInfo.templateMetadataOid == blockOid && ruleInfo.validateType == this.VALIDATION_TYPE_NULL && materialOid == ruleInfo.sxMaterialOid) {
            //this.$alert("当前区块已经配置必须为空配置，不能重复配置！");
            flag = 'Y';
          }
        }
      }
    ...
    delYZ (indexmc) {
      ...
      //   this.faModelRuleValidationList.push(this.faModelRuleValidation);
      // }
      if (this.faModelRuleValidationList[indexmc].faModelRuleValidationOid != '') {
        this.faModelRuleValidationList[indexmc].delFlag = 1;
      } else {
        this.faModelRuleValidationList.splice(indexmc, 1);
      }
    },
    ...
    getRange () {
      ...
      if ((null == this.contentDownValid || this.contentDownValid == '') && (null == this.contentUpValid || this.contentUpValid == '')) {
        ...
        if (!contentDownCheck) {
          ...
          return;
        } else if (this.contentDownValid != '' && this.contentUpValid != "") {
          if (parseInt(this.contentDownValid) > parseInt(this.contentUpValid)) {
            this.msgWarning("上限必须大于下限");
            return;
          }
        }
    ...
    //获取日期比对
    getDateCompare () {
      if (null == this.contentDateFormat || this.contentDateFormat == '') {
        this.msgWarning("日期格式不能为空");
        return;
      }
      if ((null == this.contentBeforeDateValue || this.contentBeforeDateValue == '')
        && (null == this.contentAfterDateValue || this.contentAfterDateValue == '')
        && (null == this.noMoreThanAge || this.noMoreThanAge == '')) {
        this.msgWarning("小于时间和大于时间和不超过年龄不能同时为空");
        return;
      }
      var explain1 = "输入日期格式和比对日期后，选择需要进行验证的区块。日期格式：" + this.contentDateFormat + ";比对内容：";
      if (null != this.contentBeforeDateValue && this.contentBeforeDateValue != '') {
        explain1 = explain1 + "时间不能小于" + this.contentBeforeDateValue + ";";
      }
      if (null != this.contentAfterDateValue && this.contentAfterDateValue != '') {
        explain1 = explain1 + "时间不能大于" + this.contentAfterDateValue + ";";
      }
      if (null != this.noMoreThanAge && this.noMoreThanAge != '') {
        explain1 = explain1 + "年龄不超过" + this.noMoreThanAge + ";";
      }
      ...
      /*if(null==this.contentDateFormat||this.contentDateFormat==''){
        ...
        if((null==this.contentBeforeDateValue||this.contentBeforeDateValue=='')
          &&(null==this.contentAfterDateValue||this.contentAfterDateValue=='')
          &&(null==this.biforeNowDayValue||this.biforeNowDayValue=='')
          &&(null==this.afterNowDayValue||this.afterNowDayValue=='')){
          this.msgWarning("小于时间和大于时间和距离今天时间小于天数和距离今天时间大于天数不能同时为空");
        }else{
          this.explain=" 输入日期格式和比对日期后，选择需要进行验证的区块。日期格式："+this.contentDateFormat+";比对内容："+this.contentBeforeDateValue;
          this.openDateCompare = false;
        }
      }*/
    },
