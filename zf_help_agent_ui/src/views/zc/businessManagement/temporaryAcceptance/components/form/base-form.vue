...
export default {
  ...
  methods: {
    async init () {
      await this.queryQlCaseInfo(this.caseOid);
    ...
    //查询保存的办件数据 并回填数据
    queryQlCaseInfo (caseOid) {
      return queryAllCaseByOid(caseOid).then(response => {
        ...
        this.ruleForm = {
          ...
          applyUserType: qlCaseApplay.applyUserType,
          ...
          _rqbzDueDate: qlCase.rqbzDueDate,
          bussVenueDistrictOid: qlCaseApplay.bussVenueDistrictOid
            .split(",")
            .filter(item => item)
        };
