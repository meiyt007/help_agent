...
export const formatSituationCheckList = (list, isHot = false) => {
    return list.map(item => {
        // 筛选出选中的
        const isSelectedList = item.sxServiceOptionVals.filter(i => i.isSelected == '1');
        // 是单选就字符串 多选就数组
        ...
        return {
            ...
            answers: item.sxServiceOptionVals?.map?.(child => {
                return {
                    id: child.id,
...
export const handleDefaultFlag = (list) => {
    return list.reduce((prev, cur) => {
        ...
        const isFlag = cur.answers.some(item => item.defaultFlag === 1 && (!cur.isSelected || cur.isSelected.length === 0));

        if (isFlag) {
            prev.push({ ...cur, answers: cur.answers.filter(item => item.defaultFlag === 1 && (!cur.isSelected || cur.isSelected.length === 0)) })
        }
...
export const checkAllSituationsSelected = (list) => {
    return (list.filter(item => item.fillFlag === 1).filter(item => !item.isSelected || item.isSelected.length === 0)?.[0]) ?? {};
}
 ...
 * @param {array} list 情形选项列表
 */
export const getQlTitleValList = (list, situationOid) => {
    return list.reduce((prev, cur) => {
        // 单选
        if (cur.moreStatus === 0 && !!cur.isSelected) {
            prev.push({ titleOid: cur.oid, valueOid: cur.isSelected, situationOid });
        }

        // 多选
        if (cur.moreStatus === 1 && cur.isSelected.length > 0) {
            prev = prev.concat(cur.isSelected.map(item => ({ titleOid: cur.oid, valueOid: item, situationOid })));
        }
        return prev;
    }, []);
}