<template>
  ...
  >
    <div
      class="title"
      slot="title"
      style="width:100%;color:#ffffff;overflow: hidden;text-overflow:ellipsis;white-space: nowrap"
      :title="serviceName"
    >
      {{ serviceName }}
    </div>
    ...
    <div class="breadcrumb">
      <div class="breadcrumb-container">
        <div
          class="breadcrumb-item-oneline-bg"
          :class="{ breadcrumbItemSixBg: clzsFlag }"
        />
        <div
          class="breadcrumb-item-oneline"
          :class="{ breadcrumbItemSix: clzsFlag }"
    ...
    <div class="footFlex" v-show="componentIndex!=5 &&  componentIndex!=6">
      <flexButton :serviceOid="serviceOid" :componentIndex="componentIndex"></flexButton>
    </div>
  </el-dialog>
</template>
...
import intelligentPrequalification from '../components/intelligentPrequalification.vue' //秒批秒办智能审核
import businessSubmission from '../components/businessSubmission.vue' //秒批秒办办件提交
import ConsultationTelephone from "../components/consultationTelephone.vue";
import flexButton from "../components/flexButton.vue"; //底部弹出按钮
...
export default {
  ...
  data() {
    return {
      ...
      isQlCaseChanged: true,
      rqbzMaterialNames: "",
      rqbzDueDate: "", // 容缺补正时间
      lastComponentId: null, // 点击上一步的id
      fillUserInfo:{}, // 用户录入信息
      componentIndex:1 //组件序号
    };
  },
  computed: {
    ...mapGetters(["clzsFlag"]),
    ...
    zIndex() {
      return document.querySelector(".el-dialog.common-dialog").parentElement
        .style.zIndex;
    }
  },
  watch: {
  ...
  created() {
    ...
    console.log(this.setBreadcrumb(), "setBreadcrumb");
    /** 关闭智能审核 */
    if (!this.clzsFlag) {
...
const bannerList =  this.isTempComponent? breadCrumbTemp : breadCrumbBootStrap
  if(this.handleType==="2"){
    ...
    setData(data) {
      ...
      this.caseMaterialOids = data.caseMaterialOids;
      this.createDate = data.createDate;
      this.fillUserInfo = data.fillUserInfo?data.fillUserInfo:{}
    },
