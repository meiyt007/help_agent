...
<script>
  import { getMaterialAttaByAttaOid,getMaterialPdf } from "@/api/zc/businessManagement/caseAtta";
    ...
    methods: {
      ...
      getAttaNginxPath(){
        if(!this.attaNginxPath){
          if(this.attaOid){
            getMaterialAttaByAttaOid(this.attaOid).then(response => {
      ...
      preview(ty,name,fileUrl) {
        ...
        if(1 === ty){
          ...
          this.fastFlag ='0';
          this.fileUrl = process.env.VUE_APP_SX_ROUTE_PATH + '/sxSys/file/downloadFile/'+ fileUrl;
        }
      ...
      getPdfCode (fileOid) {
        let that = this;
        getMaterialPdf(fileOid).then(response => {
