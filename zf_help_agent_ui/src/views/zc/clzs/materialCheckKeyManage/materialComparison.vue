...
<script>
import {downloadSysAtta, querySysAttaListByMaterialOid} from "@/api/zc/clzs/materialCheckKeyManage/materialCheckKeyManage.js";
...
export default {
  name: "Home",
  components: {pdf},
  props: ['sampleInfoOid','comboDirectoryOid','materiaOid','fileViewurl','caseOid'],
  data() {
    return {
      labelPosition: "top",
      activeName: "first",
  ...
  created() {
    ...
    //this.csUrl="http://172.168.252.42:9090/sysAttaService/download?attaOid=9ff665d6608d4564a9fd94e728287a73";
/*    let attaListNew=[
      {url:"http://storage.xuetangx.com/public_assets/xuetangx/PDF/PlayerAPI_v1.0.6.pdf",name:"PlayerAPI_v1.0.6.pdf",pageNum: 1, pageRotate:0,pageTotalNum:1,type:'pdf'},
      {url:"/HELP_DOC.pdf",name:"HELP_DOC.pdf",pageNum: 1, pageRotate:0,pageTotalNum:1,type:'pdf'},
      {url:"http://172.168.252.221:8888/group1/M00/00/73/rKj83V-11RCAGk4DAAGz8a4k-cw547.png",name:"图片样式",pageNum: 1, pageRotate:0,pageTotalNum:1,type:'png'}];
      this.attaList=attaListNew;
      let attoid="9ff665d6608d4564a9fd94e728287a73";*/
  /* downloadSysAtta(attoid).then(response => {
      console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
     console.log(response.data);
      console.log("###############")
      this.csUrl=response.data;
    })*/
  ...
  methods: {
    getSysAttaList() {
     /* http://172.168.252.42:9090/sysAttaService/download?attaOid=9ff665d6608d4564a9fd94e728287a73*/
      ...
      this.loading = true;
      let atta={};
      querySysAttaListByMaterialOid(materialOid,caseOid).then(response => {
        const previewUrl=response.data.data.previewUrl;
        let dataList=response.data.data.attaLat;
        let j=0;
        for (let i=0;i<dataList.length;i++){
          atta={};
          let type=this.iconByType(dataList[i].originName);
          ...
          if(type=='jpg'||type=='JPG'||type=='jpeg'||type=='JPEG'||type=='png'||type=='PNG'||type=='PDF'||type=='pdf'){
            ...
            atta.pageRotate=0;
            atta.pageTotalNum=1;
            atta.name=dataList[i].originName;
            let  filePath=dataList[i].filePath;
            let  name=dataList[i].name;
            atta.url=previewUrl+"?attaOid="+dataList[i].attaOid;
