...
<template>
  <div class="el-table__header-wrapper dialog-table require-table">
    ...
    <table cellspacing="0" cellpadding="0" border="0">
      ...
      <template  v-if="item.materialCatalogList.length<=0">
          <td>{{ index+1 }}</td>
        ...
        <tr v-for="(materialCatalog,indexx) in item.materialCatalogList" >
          ...
          <td>
              ...
              <template v-if="materialCatalog.template !=null">
                <ul  v-for="(faModelTemplateBlock,indexmc) in materialCatalog.faModelTemplateBlockList">
                  <li @click="addYZ(index,indexx,indexmc,item.serviceOid,item.materialName,materialCatalog.catalogName,item.materialOid,materialCatalog.materialCatalogOid,faModelTemplateBlock.faModelTemplateBlockOid,faModelTemplateBlock.blockName,faModelTemplateBlock.blockCode )">
                   <span>
          ...
          <td>
                  <template v-if="materialCatalog.template !=null">
                      <template  v-for="(rule,indexmc) in faModelRuleValidationList">
                        <template v-if="rule.validateType === VALIDATION_TYPE_NOTNULL && rule.delFlag===0 && rule.catalogOid===materialCatalog.materialCatalogOid &&rule.sxMaterialOid==item.materialOid ">
          ...
          <td>
              <template v-if="materialCatalog.template !=null">
                <template  v-for="(rule,indexmc) in faModelRuleValidationList">
                  <template v-if="rule.validateType === VALIDATION_TYPE_NULL && rule.delFlag===0 && rule.catalogOid===materialCatalog.materialCatalogOid &&rule.sxMaterialOid==item.materialOid">
                    <div class="require-item"   @click="delYZ(indexmc )">
                      ● {{rule.templateMetadataName }}（{{rule.templateMetadataCode}}）
                      <el-button
                      type="default"
          ...
          <td>
              <template v-if="materialCatalog.template !=null">
                <template  v-for="(rule,indexmc) in faModelRuleValidationList">
                  <template v-if="rule.validateType === VALIDATION_TYPE_CONTENT && rule.delFlag===0 && rule.catalogOid===materialCatalog.materialCatalogOid &&rule.sxMaterialOid==item.materialOid">
                    <div class="require-item"   @click="delYZ(indexmc )">
          ...
          <td>
              <template v-if="materialCatalog.template !=null">
                <template  v-for="(rule,indexmc) in faModelRuleValidationList">
                  <template v-if="rule.validateType === VALIDATION_TYPE_STR_CONTAIN && rule.delFlag===0 && rule.catalogOid===materialCatalog.materialCatalogOid &&rule.sxMaterialOid==item.materialOid">
                    <div class="require-item"   @click="delYZ(indexmc )">
                      ● {{rule.templateMetadataName }}({{rule.templateMetadataCode}})=={{rule.thanContent}}
                      <el-button
                      type="default"
          ...
          <td>
              <template v-if="materialCatalog.template !=null">
                <template  v-for="(rule,indexmc) in faModelRuleValidationList">
                  <template v-if="rule.validateType === VALIDATION_TYPE_RANGE && rule.delFlag===0 && rule.catalogOid===materialCatalog.materialCatalogOid &&rule.sxMaterialOid==item.materialOid">
                    <div class="require-item"   @click="delYZ(indexmc )">
                      ● {{rule.templateMetadataName }}({{rule.templateMetadataCode}})
          ...
          <td>
              <template v-if="materialCatalog.template !=null">
                <template  v-for="(rule,indexmc) in faModelRuleValidationList">
                  <template v-if="rule.validateType === VALIDATION_TYPE_DATE_COMPARE && rule.delFlag===0 && rule.catalogOid===materialCatalog.materialCatalogOid &&rule.sxMaterialOid==item.materialOid">
                    <div class="require-item"   @click="delYZ(indexmc )">
                      ● {{rule.templateMetadataName }}({{rule.templateMetadataCode}})
          ...
          <td>
              <template v-if="materialCatalog.template !=null">
                <template  v-for="(rule,indexmc) in faModelRuleValidationList">
                  <template v-if="rule.validateType === VALIDATION_TYPE_HAND_SIGN && rule.delFlag===0 && rule.catalogOid===materialCatalog.materialCatalogOid &&rule.sxMaterialOid==item.materialOid">
                    <div class="require-item"   @click="delYZ(indexmc )">
...
<div slot="footer" class="dialog-footer">
  <el-button type="primary"  size="medium" @click="queding()" class="save-btn">确定</el-button>
...
import {initFaModelRule,initAll,saveOrUpdate} from "@/api/zc/clzs/modelManagement/faModelRuleValidation.js";
export default {
 props: ['serviceOid'],
  data() {
    return {
  ...
  methods: {
    ...
    //添加验证
    addYZ(index,indexx,indexmc,serviceOid,materialName,catalogName,materialOid,materialCatalogOid,faModelTemplateBlockOid,blockName,blockCode ){
      let sfczFlag='N';
          ...
          var json = JSON.stringify(this.faModelRuleValidation);
           ...
           /*   this.faModelRuleValidation.delFlag=0;
              ...
              if(this.validateType==this.VALIDATION_TYPE_CONTENT){//文本验证
                ...
                sfczFlag=this.ruleCheck(this.VALIDATION_TYPE_NOTNULL,faModelTemplateBlockOid,materialOid);
              } else if(this.validateType==this.VALIDATION_TYPE_NULL){//为空
                sfczFlag=this.ruleCheck(this.VALIDATION_TYPE_NULL,faModelTemplateBlockOid,materialOid);
              }
              if(sfczFlag!='Y'){
