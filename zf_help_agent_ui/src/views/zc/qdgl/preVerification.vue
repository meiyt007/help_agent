    ...
    <table class="zf-zc-table--common zf-zc-table--td-center">
      ...
      <tbody v-for="(item, index) in tableData" :key="index">
        <tr>
          ...
          <td>
            <el-input-number v-model="item.sort" placeholder :min="1" :step="1"></el-input-number>
          </td>
          <td>
...
<script>
import { uploadsxFile, downLoadFile, querySxServicePrecheckList } from "@/api/zc/qdgl/sxService";
export default {
  ...
  methods: {
    querySxServicePrecheckList () {
      this.$getResponse(querySxServicePrecheckList({ serviceOid: this.serviceOid, }), (error, res) => {
        if (error || res.code !== 200) return;
        this.tableData = res.data||[];
      })
    },
    viewFile(item){
      this.fileUrl="/case-api/pic/previewImage?fastdfsNginxUrl="+ item.attaUrl;
      this.openInit=true;
    },
    handleAdd () {
