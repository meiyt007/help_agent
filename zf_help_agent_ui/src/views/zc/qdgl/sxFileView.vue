<template>
  <div style="height: 100%">
    <div v-if="showImage == true" class="dialog-body-content-base-style">
      <img width="300px" height="100%" style="width: 100%" :src="fileUrl" />
    </div>
    ...
    >
      <div v-if="fastFlag == '0'" class="el-button-center">
        ...
        <el-button-group>
          <el-button type="primary" @click="changePre" icon="el-icon-arrow-left"
            >上一页</el-button
          >
          <el-button type="primary" @click="changeNext"
      ...
      <embed v-if="fastFlag == '1'" :src="pdfSrc" width="100%" height="100%" />
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getAttaNginxPath() {
      if (!this.attaNginxPath) {
        if (this.attaOid) {
          getSxSysAttaByOId(this.attaOid).then(response => {
            let attaNginxPath = response.data.filePath;
    ...
    preview(ty, name, fileUrl) {
      ...
      if (1 === ty) {
        ...
        if (fileUrl.indexOf("http") > -1) {
          this.fileUrl = "/case-api/pic/previewImage?fastdfsNginxUrl=" + fileUrl;
        } else {
          this.fileUrl =
            "/case-api/pic/previewImage?fastdfsNginxUrl=" + fileUrl;
        }
        this.fastFlag = "1";
      } else {
        this.fastFlag = "0";
      ...
      if (
        ...
        this.$emit("father-colose");
        this.msgWarning("当前文件暂不支持预览");
        //this.showDoc = true
      } else if (type.indexOf("pdf") != -1) {
        if (1 === ty) {
          this.pdfSrc = "/case-api/pic/previewImage?fastdfsNginxUrl=" + fileUrl;
        } else {
