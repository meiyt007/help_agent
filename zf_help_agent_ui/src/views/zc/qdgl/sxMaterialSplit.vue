          ...
          <tr>
            ...
            <td colspan="3">
              ...
              <el-form-item v-if="form.materialSampleOid">
                ...
                <el-link
                  type="primary"
                  @click="downLoadFileInfo(form.materialSampleOid)"
                  >下载</el-link
                >
                |
                <el-link
                  type="primary"
                  @click="delFlowFile(form.materialSampleOid)"
                  >删除</el-link
                >
              </el-form-item>
            </td>
          ...
          <template v-for="(data, index) in form.reviewPointsList">
            <tr style="height: 20px">
              <td>
                <el-form-item>
                  <el-input-number
                    v-model.trim="data.serialNumber"
                    :min="1"
                    :max="9999"
                  ></el-input-number>
                </el-form-item>
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="zf-zc-table">
          ...
          <tr>
            ...
            <td colspan="3">
              ...
              <template v-if="form.materialType === 3">
                原件或复印件
              </template>
            </td>
          </tr>
          <tr>
            <td><b>来源渠道：</b></td>
            <td colspan="3">
              <template v-if="form.materialSource == 0">
                申请人自备
              </template>
              <template v-if="form.materialSource == 1">
                政府部门核发
              </template>
              <template v-if="form.materialSource == 2">
                其它
              </template>
            </td>
          </tr>
          <tr>
            <!--            <td><b>是否必要：</b></td>
            <td>
              <template v-if="form.needStatus === 0"> 否 </template>
              <template v-if="form.needStatus === 1"> 是 </template>
            </td>-->
            <td><b>排序号：</b></td>
            <td colspan="3">
              {{ form.serialNumber }}
...
<script>
import {
  pageMaterialSplitList,
  uploadsxFile,
  downLoadFile,
  saveOrUpdateMaterialXh,
  getRefinedMaterialDetail,
  deleteMaterialSplit,
  saveOrUpdateMaterialSample
} from "@/api/zc/qdgl/sxService.js";
export default {
  name: "SxMaterialSplit",
  components: {},
  props: ["serviceOid", "parentMaterialOid"],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      // 表单参数
      form: {
        materialOid: this.parentMaterialOid,
        materialType: 0,
        materialSource: "0",
        needStatus: 0,
        reviewPointsList: [],
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getRefinedMaterialDetail(row.oid).then(res => {
        if (res.code == 200) {
          ...
          this.form.materialSampleOriginName =
            res.data.materialSampleOriginName;
        }
      });
    },
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          let isFlag = false;
          this.form.reviewPointsList.forEach(item =>{
            if(item.reviewPoints == null || item.reviewPoints == '') {
              isFlag = true;
            }
          })
          if(isFlag) {
            this.$message.warning("审查要点不能为空");
            return;
          }
    ...
    getList() {
      this.loading = true;
      pageMaterialSplitList(this.parentMaterialOid).then(response => {
