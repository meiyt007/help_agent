  ...
  <div class="app-container" id="app-container-tep">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="模板名称" prop="docxTemplateName">
        <el-input v-model="queryParams.docxTemplateName" placeholder="请输入模板名称" clearable size="small" />
      </el-form-item>
      <el-form-item label="存储对象名称" label-width="120px">
        <el-input v-model="formObject.objectName" size="small" readonly />
      </el-form-item>
    ...
    <!-- 添加或修改信息对话框 -->
    <el-dialog v-dialog-drag :close-on-click-modal="false" :title="title" :visible.sync="openInit" width="70%"
               height="70vh" top="15vh" append-to-body>
      <div class="el-table__header-wrapper dialog-table">
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="0px" class="demo-ruleForm">
          ...
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__body">
            ...
            <tr>
              <td><i class="require">*</i><b>预览类型：</b></td>
              <td colspan="3" >
                <el-form-item prop="isPdf">
                  <el-col :span="24">
                    <el-select
                      v-model.trim="form.isPdf"
                      placeholder="请选择预览类型"
                      size="small" >
                      <el-option label="PDF" :value="1" ></el-option>
                      <el-option label="WORD" :value="0" ></el-option>
                      <el-option label="wordAndPdf" :value="2" ></el-option>
                    </el-select>
                  </el-col>
                </el-form-item>
              </td>
            ...
            <tr>
              ...
              <td colspan="3">
                <el-form-item prop="docxAttaOid">
                  ...
                  <el-upload class="upload-demo" accept=".docx" :headers="getToken()" :action="uploadUrl()"
                             :on-error="uploadError" :before-upload="beforeUpload" :file-list="fileList"
                             :on-success="uploadSuccess">
                    <el-button type="success" size="mini">上传模板word<i class="el-icon-upload el-icon--right"></i>
                    </el-button>
                  </el-upload>
            ...
            <tr>
              ...
              <td colspan="3" class="object_td">
                <el-row :gutter="10" class="mb8">
                  <el-col :span="1.5">
                    <el-button type="primary" icon="el-icon-plus" size="mini" @click="addAppendObject">增加
                    </el-button>
                    <div style="margin-top: 8px">当前存储对象： <font color="red">{{ formObject.objectName }}</font>
                    </div>
                  </el-col>
                </el-row>
                  ...
                  <el-table-column label="存储对象key" width="320">
                    <template slot-scope="scope">
                      <el-form-item :prop="'appendObjectList.' + scope.$index + '.key'"
                        ...
                        <el-input placeholder="存储对象key" v-model.trim="scope.row.key" maxlength="50" show-word-limit />
                      </el-form-item>
                    </template>
                  </el-table-column>
                  <el-table-column label="关联存储对象">
                    <template slot-scope="scope">
                      <el-form-item :prop="'appendObjectList.' + scope.$index + '.objectOid'"
                        ...
                        <el-select style="width: 100%" v-model="scope.row.objectOid" clearable filterable
                                   placeholder="请选择">
                          <el-option v-for="append in appendObjectOptions" :key="append.objectOid"
                                     :label="append.objectName" :value="append.objectOid">
                          </el-option>
                        </el-select>
                      </el-form-item>
                    </template>
                    ...
                    <template slot-scope="scope">
                      <el-form-item align="center">
                        <el-button size="mini" type="text" icon="iconfont zfsoft-shanchu"
                                   @click="deleteAppendObject(scope.$index)">删除
                        </el-button>
                      </el-form-item>
                    </template>
                  </el-table-column>
            ...
            <tr>
              ...
              <td class="tipTd" colspan="3" v-pre>
                ...
                <br />
                <span
                      style="color: red">3.图片标签以%开始：{{%var}}，并支持{{%var_height_width}}设置高和宽，支持多图片。如:{{%var_400_300}}</span>
                <br />
        ...
        <el-form :model="form" label-width="0px" class="demo-ruleForm">
          <table cellspacing="0" cellpadding="0" border="0" class="el-table__body">
            ...
            <tr>
              ...
              <td v-if="form.isPdf == '0'" colspan="3">
                WORD
              </td>
              <td v-if="form.isPdf == '1'" colspan="3">
                PDF
              </td>
              <td v-if="form.isPdf == '2'" colspan="3">
                wordAndPdf
              </td>
            </tr>
            <tr>
...
<script>
import { save, del, getOne, getPage, able, getAppendObjects, getObjectColumns } from "@/api/form/docxTemplate"
...
export default {
  ...
  methods: {
    ...
    viewFile(attaOid) {
      ...
      let item = {
        show: true, attaOid: attaOid, name: this.form.attaName
