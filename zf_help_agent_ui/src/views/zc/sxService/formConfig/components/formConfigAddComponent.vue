...
// api
import { queryFormInfoExistList, save, del, saveOrUpdateField } from "@/api/zc/sxService/serviceFormConfig/formDesign";

import { querySxFillLabelList, querySxFieldTypeListByServiceOid } from "@/api/zc/sxService/serviceFormConfig/formFillLabel";
...
export default {
  ...
  methods: {
    getFieldTypeList () {
      return querySxFieldTypeListByServiceOid({ serviceOid: this.serviceOid }).then(({ code, data }) => {
        if (code === 200) {
          this.fieldTypeList = (data || []).map(item => ({
    ...
    handleClassify (value) {
      ...
      this.handleCreateTable(value, async () => {
        const [err, ret] = await awaitPromise(querySxFillLabelList({ serviceOid: this.serviceOid, typeOid: value.fieldTypeOid }));
        if (err || !ret) return;

        const { data, code } = ret;
        if (code !== 200) return;

        if (!data || !data.length) return this.$message.warning('未查询到分类标签信息');
    ...
    // 设置存储
    handleCreateTable (value, callback = noop) {
      ...
      return queryFormInfoExistList(this.serviceOid, value.fieldTypeOid)
        .then(({ data, code }) => {
          if (code === 200) {
            if (data?.length > 0) {
              ...
              return this.$message.warning("分类已存在设计表单");
            }
            callback();
          }
        });
    ...
    handleSave (data, loading, logicFormName, list) {
      ...
      saveOrUpdateField({
        ...
        logicFormName,
        //status: 3  20221021 赵冰峰修改  点击保存的时候固定status为3，但是前端查询的时候只查5的表单，所以修改
        status: 5,
    ...
    async handleIsModifyData () {
      ...
      const [err, ret] = await awaitPromise(querySxFillLabelList({ serviceOid: this.serviceOid, typeOid: mainTable.fieldTypeOid }));
      if (err || !ret) return;
