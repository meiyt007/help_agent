...
<script>
  import {getPdf } from "@/api/form/atta";
  ...
  export default {
    ...
    name: "FormFileView",
    props:["attaOid","name","attaNginxPath","downloadFileUrl"],
    data() {
      return {
        flag:true,
        showDoc: false,
    ...
    methods: {
      ...
      getAttaNginxPath(){
        ...
        if(this.attaOid){
          this.preview(0,this.name,this.attaOid);
        }
      },
      preview(ty,name,fileUrl) {
        ...
        if(1 === ty){
          ...
          if(this.downloadFileUrl){
            this.fileUrl = this.downloadFileUrl + fileUrl + '?access_token=' +  getToken();
          }else {
            this.fileUrl = process.env.VUE_APP_FORM_API_ROUTE_PATH + "/manager/downloadFile/"+ fileUrl + '?access_token=' +  getToken();
          }
        }
