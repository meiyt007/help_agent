  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="bussinfoList" border>
      ...
      <el-table-column
        ...
        :show-overflow-tooltip="true"
      />
      <el-table-column label="办理时限" align="center" prop="timeLimit" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span>{{scope.row.timeLimit + (scope.row.timeUnit == 'N' ? '自然日' : '工作日')}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="设计标识" align="center" width="80" prop="deploymentId">
        <template slot-scope="scope">
          <span v-if="null == scope.row.modelId">未设计</span>
          <span v-if="null != scope.row.modelId">
            <span class="redSpan" v-if="null==scope.row.isPublish || '0'==scope.row.isPublish">未部署</span>
            <span class="greenSpan" v-if="null!=scope.row.isPublish && '1'==scope.row.isPublish">已部署</span>
          </span>
        </template>
      </el-table-column>
      <el-table-column label="新增时间" align="center" prop="createDate" :show-overflow-tooltip="true" />
    ...
    >
      ...
      >
        <table cellspacing="0" cellpadding="0" border="0" class="el-table__body">
          ...
          <tr>
            <td>
              <b>
                <i class="require" v-if="!viewFlag">*</i>所属区划：
              </b>
            </td>
            <td>
              <el-form-item prop="districtOid">
                <el-col :span="24">
                  <treeselect
                    ...
                    @input="(id, instanceId) => districtSel1(id, () => form.organOid = null)"
                    :disabled="viewFlag"
                  />
                </el-col>
              </el-form-item>
          ...
          <tr>
            <td>
              <b>
                <i class="require" v-if="!viewFlag">*</i>流程名称：
              </b>
            </td>
            <td colspan="3">
          ...
          <tr>
            <td>
              <b>
                <i class="require" v-if="!viewFlag">*</i>流程编码：
              </b>
            </td>
            <td>
            ...
            <td>
              <b>
                <i class="require" v-if="!viewFlag">*</i>办理时限：
              </b>
            </td>
            <td>
              ...
              <el-form-item prop="timeLimit" v-if="viewFlag">
                <el-col :span="24">
                  <el-form-item v-if="viewFlag">
                    ...
                    >{{form.timeLimit + ' ' + (form.timeUnit == 'N' ? '自然日' : '工作日')}}</span>
                  </el-form-item>
                </el-col>
              </el-form-item>
            </td>
          ...
          <tr>
            <td>
              <b>
                <i class="require" v-if="!viewFlag">*</i>所属应用：
              </b>
            </td>
            <td>
          ...
          <tr>
            ...
            <td colspan="3">
              <el-form-item prop="workflowMemo">
                <el-col :span="24">
                  <el-input v-model="form.workflowMemo" type="textarea" v-show="!viewFlag" maxlength="1000" show-word-limit placeholder="请输入备注"></el-input>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button v-if="!viewFlag" type="primary" @click="submitForm">确 定</el-button>
        <el-button @click="() => {addDialogShow = false}">{{viewFlag ? '关闭' : '取 消'}}</el-button>
      </div>
    </el-dialog>
    ...
    <!--流程复制弹窗-->
    <el-dialog :close-on-click-modal="false" title="流程复制" v-if="copyDialogShow" :visible.sync="copyDialogShow" width="900px" append-to-body>
      <el-form :model="queryParams1" ref="queryForm1" :inline="true" label-width="68px">
        ...
        <el-form-item class="fr mr0">
          ...
          <el-button type="warning" icon="el-icon-refresh" size="mini" @click="resetQuery1" class="ml5">重置</el-button>
        </el-form-item>
      </el-form>
    ...
    <!--历史流程弹窗-->
    <el-dialog :close-on-click-modal="false" title="业务历史流程信息" v-if="hisDialogShow" :visible.sync="hisDialogShow" width="900px" append-to-body>
      <el-table v-loading="loading2" :data="bussinfoList2" border="">
        ...
        <el-table-column label="版本" width="80" align="center" prop="version" :show-overflow-tooltip="true"/>
        <el-table-column label="创建时间" width="180" align="center" prop="deployTime" :show-overflow-tooltip="true"/>
