  ...
  export default {
    ...
    methods: {
        ...
        this.$confirm("是否确认清空所有链接?", "警告", {
          ...
          .then(function () {
            ...
            processArr.forEach(function (reg, index) {
              ...
              if (reg.type == "bpmn:UserTask") {
                reg.businessObject.$attrs.rejectTaskId = '';
                reg.businessObject.$attrs.rejectWaypoints = [];
                reg.businessObject.$attrs.fallbackTaskId = '';
                reg.businessObject.$attrs.fallbackWaypoints = [];
              }
            });
          })
          .catch(function () {});
      ...
      checkbpmnModelerXML() {
        ...
        console.log(callActivityArr)
        if (userEventArr.length == 0 && callActivityArr.length == 0) {
          ...
          for (var i = 0; i < userEventArr.length; i++) {
            ...
            if (undefined != handleUserType && "2" != handleUserType) {
              ...
              if ((undefined == handleUserOids || '' == handleUserOids) && (undefined == handlePostOids || '' == handlePostOids)) {
