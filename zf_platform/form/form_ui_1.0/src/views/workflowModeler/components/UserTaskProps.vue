<template>
  <div>
    ...
    <el-form-item label="人员设定类型">
      <el-radio-group v-model="handleUserType" @change="userTypeChange">
        <el-radio label="1">环节自定义</el-radio>
        <el-radio label="2">上一环节设置</el-radio>
      </el-radio-group>
      <div style="color: red">备注：上一环节设置，则办理人和办理岗位需要设置空,若不为空，默认为环节自定义</div>
    </el-form-item>
    ...
    <el-form-item label="退回环节">
      <el-select v-model="fallbackTaskId" size="small" multiple @change="changeFallbackTaskId" filterable
                 clearable placeholder="请选择退回环节">
        <el-option
          v-for="p in userTaskIdList"
          :key="p.taskId"
          :label="p.taskName"
...
export default {
  ...
  methods: {
    ...
    //驳回
    changeRejectTaskId(toOids) {
      const bpmnModeler = this.bpmnModeler();
      let modeling = bpmnModeler.get("modeling");
      let that = this;
      if(0 == toOids.length){
        this.rejectTaskId = [];
        this.modeling.updateProperties(this.element, { rejectTaskId: [] });
        this.clearBackConnection('bh',this.id);
        return false;
      }
      ...
      elementRegistryMap.forEach(function (id, index) {
        ...
        if (undefined != reg && reg.type == "bpmn:SequenceFlow") {
          if (reg.businessObject.$attrs.flowClass == 'bh' && reg.businessObject.$attrs.startTaskId == that.id) {
            let regEle = [];
            regEle.push(reg)
            modeling.removeElements(regEle);
          }
        }
      });
      const startTask = elementRegistry.find((item) => item.id == this.id);
      for(let toOid of toOids){
      ...
      this.modeling.updateProperties(this.element, { rejectTaskId: toOids.toString() });
    },
    //退回
    changeFallbackTaskId(toOids) {
      const bpmnModeler = this.bpmnModeler();
      let modeling = bpmnModeler.get("modeling");
      let that = this;
      if(0 == toOids.length){
        this.fallbackTaskId =[];
        this.modeling.updateProperties(this.element, { fallbackTaskId: [] });
        this.clearBackConnection('th',this.id);
        return false;
      }
      const elementRegistry = this.bpmnModeler().get("elementRegistry");
      const canvas = this.bpmnModeler().get("canvas");
      ...
      elementRegistryMap.forEach(function (id, index) {
        var reg = elementRegistry.find((item) => item.id == id);
        if (undefined != reg && reg.type == "bpmn:SequenceFlow" && reg.businessObject.$attrs.startTaskId == that.id) {
          if (reg.businessObject.$attrs.flowClass == 'th') {
            let regEle = [];
            regEle.push(reg)
            modeling.removeElements(regEle);
          }
      ...
      this.modeling.updateProperties(this.element, {
        fallbackTaskId: toOids.toString(),
      });
    },

    clearBackConnection(type,clickId){
      const bpmnModeler = this.bpmnModeler();
      let modeling = bpmnModeler.get("modeling");
      const elementRegistry = this.bpmnModeler().get("elementRegistry");
      const canvas = this.bpmnModeler().get("canvas");
      ...
      elementRegistryMap.forEach(function (id, index) {
        var reg = elementRegistry.find((item) => item.id == id);
        if (undefined != reg && reg.type == "bpmn:SequenceFlow") {
          if (reg.businessObject.$attrs.flowClass == type && reg.businessObject.$attrs.startTaskId == clickId) {
            let regEle = [];
            regEle.push(reg)
            modeling.removeElements(regEle);
            //modeling.removeShape(labelFlow);
  ...
  watch: {
        ...
        if (newVal.type == "bpmn:UserTask") {
          ...
          this.handlePostNames =
            businessObject.$attrs.handlePostNames != undefined
              ...
              : [];
          let rejectTaskIdStr = businessObject.$attrs.rejectTaskId != undefined
            ? businessObject.$attrs.rejectTaskId : "";
          if('' != rejectTaskIdStr){
            this.rejectTaskId = rejectTaskIdStr.split(',');
          }
          let fallbackTaskIdStr = businessObject.$attrs.fallbackTaskId != undefined
            ? businessObject.$attrs.fallbackTaskId : "";
          if('' != fallbackTaskIdStr){
            this.fallbackTaskId = fallbackTaskIdStr.split(',');
          }
          this.linkOid =
    ...
    rejectTaskId: {
      handler(newVal, oldVal) {
        ...
        //alert(newVal)
        if(undefined != this.modeling){
          //console.log('1123',newVal.toString())
          //this.modeling.updateProperties(this.element, { rejectTaskId: newVal.toString() });
        }
