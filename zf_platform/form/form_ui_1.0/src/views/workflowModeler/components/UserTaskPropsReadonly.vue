<template>
  <div class="user-task-props-readonly">
    ...
    <el-form-item label="驳回环节">
      ...
      <span v-for="p in userTaskIdList" v-if="rejectTaskId.indexOf(p.taskId) > -1">{{p.taskName}} </span>
    </el-form-item>
    <el-form-item label="退回环节">
      <span v-if="'' == fallbackTaskId || null== fallbackTaskId ">无</span>
      <span v-for="p in userTaskIdList" v-if="fallbackTaskId.indexOf(p.taskId) > -1">{{p.taskName}} </span>
    </el-form-item>
...
<script>
  import {
    getWorkflowBussFlowStepByProcessDefIdAndActivityId,getUserNames
        ...
        getWorkflowBussFlowStepByProcessDefIdAndActivityId(data).then((resp) => {
          ...
          that.handleUserNames =
            step.handleUserNames != undefined
              ...
              : '无';
            let handleUserOids =
            step.handleUserOids != undefined
              ? step.handleUserOids
              : '';
          if(this.handleUserNames == '' && '' != handleUserOids){
            getUserNames(handleUserOids).then((resp) => {
              this.handleUserNames = resp.data;
            });
          }
          ...
          if (newVal.type == "bpmn:UserTask") {
            ...
            let handleUserType =
              businessObject.$attrs.handleUserType != undefined
                ...
                : '';
            if(''==handleUserOids && ''==handlePostOids && '2'!=handleUserType){
              //为空，可能是历史流程
              ...
              this.handleUserNames =
                businessObject.$attrs.handleUserNames != undefined && businessObject.$attrs.handleUserNames != ''
              ...
              this.handlePostNames =
                businessObject.$attrs.handlePostNames != undefined && businessObject.$attrs.handlePostNames != ''
