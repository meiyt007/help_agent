...
//        Parameter[] parameters = method.getParameters();
        ...
        // java 8 支持参数编译不变为 arg0...
        Map fieldValueMap = ReflectionUtil.getFieldsName(pjp);
        for (int i = 0; i < parameterAnnotations.length; i++) {
            ...
            if (parameterAnnotations[i].length == 0) { //说明该参数没有注解，此时该参数可能是实体类，也可能是Map，也可能只是单参数
                ...
                } else if(ObjectUtil.isPackageObject(object)) {
                    map.putAll(objectToMap(object));
                    System.out.println("该对象为用户自定义的对象");
                } else if(!CollectionUtils.isEmpty(fieldValueMap)) {
    ...
    /**
     * 利用 jsqlparser 转化sql中的下划线为驼峰规则
     * @author kkfan
     * @param sql 转化前的sql
     * @return
     * @throws JSQLParserException
     */
    public static String convertSQLUnderscore(String sql) throws JSQLParserException {
        logger.debug(MessageFormat.format("{0}：转化前的sql为：{1}", Thread.currentThread().getName(), sql));
        sql = sql.toLowerCase();
        ...
        ExpressionVisitorAdapter expressionVisitorAdapter = new ExpressionVisitorAdapter() {
            ...
            public void visit(Column column) {
                column.setColumnName(NamingConversion.underlineToHump(column.getColumnName()));
            }
        ...
        // 替换where子句
        selectBody.getWhere().accept(expressionVisitorAdapter);
        // 替换查询属性
        List<SelectItem> selectItems = selectBody.getSelectItems();
        selectItems.forEach(selectItem -> {
            selectItem.accept(expressionVisitorAdapter);
        });

        logger.debug(MessageFormat.format("{0}：转化后的sql为：{1}", Thread.currentThread().getName(), stmt.toString()));
