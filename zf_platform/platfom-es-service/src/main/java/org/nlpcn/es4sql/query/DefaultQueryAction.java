...
import com.zfsoft.es.service.util.ElasticSearchUtil;
import com.zfsoft.es.service.util.NamingConversion;
...
public class DefaultQueryAction extends QueryAction {
	...
	public SqlElasticSearchRequestBuilder explain() throws SqlParseException {
		List<String> indexList = Arrays.asList(select.getIndexArr());
        Optional.ofNullable(indexList)
				.orElseGet(Lists::newArrayList)
				.forEach(index -> {
					boolean indexExist = ElasticSearchUtil.isIndexExist(index);
					if(!indexExist) {
						ElasticSearchUtil.createIndex(index);
					}
				});
        select.getFrom().forEach(item -> {
            item.setType(NamingConversion.underlineToHump(item.getIndex()));
        });
