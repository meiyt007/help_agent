...
import com.zfsoft.platform.auth.feign.SysConfigFeignService;
    ...
    public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
        ...
        if (existingAccessToken != null) {
            if (existingAccessToken.isExpired()) {
                ...
                if(200 == configApiResultSet.getCode() && BaseStaticParameter.NO.equals(configApiResultSet.getData().getValue())) {
                    tokenStore.removeAccessToken(existingAccessToken);
                } else {
                    // Re-store the access token in case the authentication has changed
                    final Map<String, Object> additionalInfo = new HashMap<>(8);
                    AuthUserDetails userDetails = (AuthUserDetails) authentication.getUserAuthentication().getPrincipal();
                    userDetails.setToken(existingAccessToken.getValue());
                    ...
                    ((DefaultOAuth2AccessToken) existingAccessToken).setAdditionalInformation(additionalInfo);
