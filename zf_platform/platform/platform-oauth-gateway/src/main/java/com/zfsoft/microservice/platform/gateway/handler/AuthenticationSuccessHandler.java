...
public class AuthenticationSuccessHandler extends WebFilterChainServerAuthenticationSuccessHandler {
    ...
    public Mono<Void> onAuthenticationSuccess(WebFilterExchange webFilterExchange, Authentication authentication) {
        ...
        //校验密码是否30天未更新  功能移到SecurityUserDetailsService中处理
        /*ApiResultSet<SysLogin> currentLogin = sysUserFeignService.getSysLoginByAccount(username);
        Date passwordDate = currentLogin.getData().getPasswordDate();
        if (passwordDate != null){
            long l = DateUtil.between(passwordDate, new Date(), DateUnit.DAY);
            if (l > 30){
                wsResponse.setPsFlag(BaseStaticParameter.Y);
            }
        }*/
        ...
        Mono<DataBuffer> dataBufferMono = exchange.getSession().flatMap(WebSession::changeSessionId).then(Mono.just(bodyDataBuffer));
