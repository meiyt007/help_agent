    ...
    <div class="depart-container">
        <div class="content">
            <el-radio-group v-model="tableradioGroup" class="chooseOption">
                <el-radio
                        :key="data.organOid" name="group1"  @change="changeService(data)" v-for="data in list" :label="data.organOid"
                            >{{ data.name  }}
                </el-radio>
            </el-radio-group>
        </div>    
      ...
      <el-dialog
        ...
        v-dialogDrag
        >
        <chooseWorker v-if="workerShow" :type="type" @closeWorker="closeWorker" :organOid="tableradioGroup"/>
        </el-dialog>
    </div>
</template>
<script>
    ...
    export default{
        ...
        mounted(){
            listOrganByDistrictAndService().then(res=>{
                this.list = res.data;
            })
        },
        methods:{
