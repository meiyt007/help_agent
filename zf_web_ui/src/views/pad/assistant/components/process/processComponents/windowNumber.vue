...
<template>
    <div class="intelligentFormFilling-container window-container" v-loading="turnServiceLoading">
      <div class="tabHeader none" v-if="basicUserInfo.groupPost === '1'">
        <p
          class="tabLeft"
          @click="changeTab('1')"
        ...
        <div class="tab" ref="tab2" v-show="tabType === '2'">
          ...
          <div class="table-btn">
            ...
            <el-radio-group v-model="tableradioGroup" class="chooseOption">
                <el-radio
                        :key="data.cateGoryId" name="group1"  @change="changeService(data)" v-for="data in allServiceList" :label="data"
          ...
          <div class="table-btn">
            <el-radio-group v-model="groupLeaderId" class="chooseOption">
              <el-radio  v-model="groupLeaderId" name="group2" :key="item.cateGoryId" v-for="item in haWorkUsers" :label="item">
              {{ item.cateGoryName }}
              </el-radio>
            </el-radio-group>
          </div>
  ...
  <script>
  import { getAllUserService, getAppointCatalogue,takeNumber,winNumbPush } from "@/api/modules/helpAgent";
  import {
    serviceTurn,
    turnIsVerify,
    turnServiceVerify,
  ...
  export default {
    ...
    methods: {
      ...
      serviceTurn() {
        ...
        takeNumber(JSON.stringify(obj)).then(res=>{
            if(res.code == 200){
                ...
                dataObj['companyCode'] = '';
                dataObj['companyName'] = '';
                that.$alert('<div>'+ res.data.tickerMsg.stNumber +'</div><div>'+res.data.tickerMsg.stDesc+'</div>', '', {
