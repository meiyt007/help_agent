            ...
            <div id="box" ref="scroll" >
                <template v-for="(data,index) in list">
                    <div class="item centerY" :class="{'item-pdf':data.materialUrl.indexOf('pdf') !== -1}" :key="index" v-if="data.materialName">
                        ...
                        <div class="look" @click="toShare(data)" v-if="data.materialName.indexOf('doc') !== -1 || data.materialName.indexOf('docx') !== -1" >分享</div>
                        <div class="look" @click="detail(data)" v-else>查看</div>
                    </div>
                </template>
            </div>
        ...
        getData() {
            ...
            queryMaterialList(this.query).then(res => {
                if (res.data.data.length > 0) {
                    ...
                    that.list =this.list.concat(res.data.data);
                } else {
                    // that.list = [];
                    // that.total = 0;
