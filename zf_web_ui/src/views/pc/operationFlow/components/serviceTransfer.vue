  ...
  <div class="intelligentFormFilling-container" v-loading="turnServiceLoading">
    ...
    <div class="body-content" v-loading="loadingData">
      <div class="tab" ref="tab1" v-show="tabType === '1'">
        <el-input placeholder="请输入帮办人员姓名" v-model="name" class="input-with-select nameQuery"></el-input>
        <el-table
          ref="table1"
          :height="tableHeight1"
          :data="queryGroupServiceList"
      ...
      <div class="tab" ref="tab2" v-show="tabType === '2'">
        ...
        <div class="table-btn">
          <el-radio v-model="tableradioGroup" :key="data.groupId" v-for="data in allServiceList" :label="data"
                @change="changeService(data)"> {{ data.groupName }} </el-radio>
        </div>
        <div class="table-title">请选择转派接收人：</div>
        <div class="table-btn">
          组长
          <el-radio
...
<script>
import { getAllUserServiceByGroupSplit, getGroupList, getGroupLeaderList } from "@/api/modules/helpAgent";
import {
  serviceTurn,
  turnIsVerify,
  turnServiceVerify,
...
export default {
    ...
    getWorkUserList() {
      ...
      getAllUserServiceByGroupSplit(data).then((res) => {
        if (res.code === 200) {
          ...
          this.queryGroupServiceList = this.groupServiceList;
          this.tableHeight1 = this.$refs.tab2.clientHeight-55;
        }
      });
    },
            ...
            if (res.code === 200) {
              ...
              if (res.data.turnIsVerify != "1") {
                ...
                if (res.data.turnIsVerify === "3") {
                  that.$message.warning(res.data.rollbackMemo);
  ...
  watch: {
    name(val){
      ...
      if(val){
        let arr = [];
        this.groupServiceList.forEach(function(data){
          if(data.name.indexOf(val) > -1){
            arr.push(data);
          }
        })
        this.queryGroupServiceList = arr;
      }else{
        this.queryGroupServiceList = this.groupServiceList;
      }
