...
<template>
    <div class="app-container">
        <div class="depart-content">
            <div class="centerY" style="padding-top:20px">
                <p>请选择联办时间：</p>
                <el-date-picker
                v-model="query.appDate"
            ...
            <el-checkbox-group v-model.trim="query.groupIds" class="chooseOption">
                <el-checkbox 
                v-for="item in list" 
                :key="item.groupId" 
                :id="item.groupId" 
...
<script>
import { getDeskGroup,appDepartment } from "@/api/modules/helpAgent";
export default {
    name: "depart",
    components: {
    },
    ...
    methods: {
        toNext(){
            ...
            if(this.query.appDate == ''){
                this.$message.warning("请选择联办时间");
                return;
            }
            this.query.groupIds = this.query.groupIds.join(",");
            let that = this;
            appDepartment(this.query).then(res=>{
                if(res.code == 200){
